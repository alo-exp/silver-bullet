...
Triage options A/B/C are correctly reproduced verbatim. Path 1A (superpowers:systematic-debugging → gsd-debug), Path 1B (sb:forensics), Path 1C (gsd-forensics) are all correctly described with their handoff logic. Step 2 TDD enforcement rule ("RED must appear before writing any fix") is accurate. Steps 3–8 match the SKILL.md with correct step numbers. Non-skippable gates are listed correctly (Step 7 = security, Step 7b = quality-gates, Step 6 = gsd-verify-work). Bottom navigation links are correct.
...
The UI workflow in SKILL.md does not have a `gsd-analyze-dependencies` step (unlike sb:feature), so the HTML correctly omits it. This is a non-issue, noted for completeness.

**Issue 3-C — Minor: reference/index.html description for /sb:ui contains invented content:**
In `site/help/reference/index.html` the Silver Bullet skills table describes `/sb:ui` as: *"Chains intel → product-brainstorm → **accessibility-review** → sb:brainstorm → quality-gates → gsd-ui-phase …"* — inserting `accessibility-review` as a step between product-brainstorm and sb:brainstorm. The `silver-ui/SKILL.md` does not include an `accessibility-review` step at all; the WCAG mention appears only in the gsd-ui-review output description. This description is in the reference page (page 13), reported there — noted here for cross-reference.

Similarly, the reference page's Orchestration Workflows table row for `/sb:ui` states the first step is *"Complexity triage → sb:intel → **accessibility-review** → product-brainstorm"* — again inserting `accessibility-review` which does not exist in the SKILL.md.
...
The silver/SKILL.md §Step 1d auto-trigger signals list a fourth: *"Change affects public API or data model fundamentally."* The HTML page omits this trigger. (The sb:feature SKILL.md also lists this as the 4th trigger; silver-bullet.md §2h also includes it in the "MultAI auto-offer" table.)
...
No step numbers are wrong. Paths 2a/2b/2c and Steps 3–4 are accurately described. The handoff options (sb:feature / sb:devops / Done) are reproduced correctly. Bottom navigation links are correct.
...
The HTML page states for Step 5 (Create release): *"Git-history release notes generation + GitHub Release creation with version tag. **Blocks if README is stale.**"* The silver-release SKILL.md does not mention this blocking condition. This constraint does appear in silver-bullet.md §3 rules: *"README.md MUST be updated before release. /silver-create-release will block if README is stale."* However it is attributed to the wrong location in the SKILL.md description — this is a rule from §3 not embedded in the sb:create-release step. The statement is not factually wrong (it's accurate per §3), but it attributes the blocking behavior to Step 5 specifically, which could mislead users about where the check lives. Low severity.
...
All 4 steps are present and accurately described. The §10 preferences note (deliberately NOT read in sb:fast) is correctly explained. The complexity triage gate (Step 0), scope expansion STOP condition (Step 2) including the verbatim banner text, and the non-trivial escalation options (sb:feature / sb:bugfix / sb:ui / sb:devops / sb:deep-research in Step 0 vs sb:feature / sb:bugfix / sb:devops / stop in Step 2) are correctly reproduced. The "When NOT to use" section matches the SKILL.md's "Not appropriate for" list exactly. Bottom navigation links are correct.
...
- `/sb:feature` → `./silver-feature.html` ✓
- `/sb:bugfix` → `./silver-bugfix.html` ✓
- `/sb:ui` → `./silver-ui.html` ✓
- `/sb:devops` → `./silver-devops.html` ✓
- `/sb:deep-research` → `./silver-deep-research.html` ✓
- `/sb:release` → `./silver-release.html` ✓
- `/sb:fast` → `./silver-fast.html` ✓

Card descriptions are accurate summaries. The blast-radius "LOW / MEDIUM / HIGH / CRITICAL" mention on the devops card is correct. The §10 preferences note on the fast card is correctly stated. Skill names are spelled with colons (sb:feature etc.) consistently.
...
- `sb:feature — Step 9: gsd-verify-work (must pass before Step 10: ship)`
...
**Issue 10-B — sb:ui verification description is incomplete/inaccurate:**
The HTML states: `sb:ui — after implementation, gsd-verify-work + accessibility check before ship`. The silver-ui SKILL.md does not include an `accessibility check` as part of the verify step — accessibility is covered in the 6-pillar gsd-ui-review (Step 9). The "accessibility check" reference here is misleading.

**Issue 10-C — sb:release exemption description is misleading:**
The HTML states verification for sb:release is *"replaced by the full quality gates suite + UAT audit + milestone audit, which are more comprehensive."* While accurate in spirit, sb:release also explicitly includes `gsd-verify-work` embedded within milestone audit (as stated in the SKILL.md non-skippable gates: "gsd-verify-work (embedded in milestone audit)"). Saying it is "replaced" is inaccurate — it still runs, embedded.
...
All 5 steps are present and accurately described. The Opus 4.6 (1M context) switch in Step 1 is correctly stated. The UNTRUSTED DATA boundary rule for docs/ is faithfully reproduced. The /compact timing (after reading, not before) is correctly explained. The 4 update checks (5.1 Silver Bullet, 5.2 GSD, 5.3 Plugins/informational, 5.4 MultAI) are all present with correct behaviors. The /sb:update 5-step flow (fetch → first confirmation → update files → second confirmation before registry write → registry write) matches the sb:update skill design. The anti-skip enforcement description is accurate. No steps are invented or missing.

---

### 13. site/help/reference/index.html
**Source:** Must contain all 7 new skills + sb:update
**Verdict: ISSUES FOUND**

**Issue 13-A — /sb:ui description inserts non-existent `accessibility-review` step:**
In the Silver Bullet Skills table (sb-skills section), the description for `/sb:ui` reads: *"Chains intel → product-brainstorm → **accessibility-review** → sb:brainstorm → quality-gates → gsd-ui-phase …"* The silver-ui/SKILL.md does not include an `accessibility-review` skill invocation anywhere. The visual audit is done by `gsd-ui-review` (Step 9) and mentions WCAG as one of its 6 pillars. Listing `accessibility-review` as a step in the chain is invented content.

**Issue 13-B — Orchestration Workflows table row for /sb:ui states wrong first step:**
The Orchestration Workflows table states the first step for `/sb:ui` as: *"Complexity triage → sb:intel → **accessibility-review** → product-brainstorm"*. The actual first step per SKILL.md is: sb:intel → [sb:explore if fuzzy] → /product-brainstorming → sb:brainstorm → gsd-ui-phase. `accessibility-review` is not a first step.

**Issue 13-C — /sb:release description invents a "post-release plugin version check" step:**
In the Silver Bullet Skills table, the description for `/sb:release` ends with: *"…post-release plugin version check."* The silver-release/SKILL.md has no such step. The 8 steps end at `gsd-complete-milestone`. This is invented content.
...
**All 7 new workflow skills + sb:update are present:** sb:feature, sb:bugfix, sb:ui, sb:devops, sb:deep-research, sb:release, sb:fast, sb:update — all 8 entries exist in the skills table. ✓
...
The card links to `workflows/` with the correct relative path. It lists sb:feature, sb:bugfix, sb:ui, sb:devops, and sb:release in the bullet topics (sb:deep-research and sb:fast are not listed but this is acceptable as it is a summary card, not an exhaustive list). The badge shows "v0.13.0" which is consistent with the workflows being new in that release.
...
| 10-A | verification.html | Medium | sb:feature step numbers wrong — says "Step 9" for gsd-verify-work; SKILL.md says Step 8 |
| 10-B | verification.html | Low | sb:ui verify description invents an "accessibility check" step not in the SKILL.md |
| 10-C | verification.html | Low | sb:release exemption says verification is "replaced" — it still runs (embedded in milestone audit) |
| 13-A | reference/index.html | Medium | /sb:ui skills table description inserts non-existent `accessibility-review` step |
| 13-B | reference/index.html | Medium | Orchestration Workflows table: /sb:ui first step shows `accessibility-review` (not in SKILL.md) |
| 13-C | reference/index.html | Low | /sb:release description adds invented "post-release plugin version check" step |
...
- **Most critical issues:** The invented `accessibility-review` step appearing in two places on the reference page (13-A, 13-B), the wrong step number for gsd-verify-work on the verification concepts page (10-A), the stale quality-gates count that needed correction to "8 core dimensions plus conditional gates" (13-D), and the missing Step 16 on the sb:ui page (3-A)
...
- **Skill names are correctly spelled** throughout all pages (sb:feature, sb:bugfix, etc.)
