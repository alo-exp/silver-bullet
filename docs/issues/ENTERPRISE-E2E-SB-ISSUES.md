...
| E2E-002 | open→fixed | harness | Context-full recovery used `/clear` (destructive) | `recover_from_context_exhaustion` now sends `/compact`; matrix fresh-row `/clear` unchanged |
...
| E2E-089 | friction→fixed | harness/cursor | Retry2 row logs 0–317B (timeout/connection noise); matrix evidence PASS but outcome FAIL; row 20 timeout; parallel codex batch regressed rows 9–10–13–17 outcome files | Headless log streaming in `cursor/agent.sh`; matrix evidence resolver + hook/heal/super scorer pass when evidence/worker-completion present (watch blocker demoted); `retry2-rescore.sh` → **22/22** @pending |
...
| E2E-022 | friction | matrix | Row `/clear` at session start is intentional (fresh context per row) | Distinct from context-full `/compact` recovery |
...
|---------|---------|-----------|
| Context window full (harness stall) | `/compact` | Preserves SB state markers and session continuity |
