...
Expected: FAIL on the new sidekick marker case because the hook still reads `~/.claude/.silver-bullet/state`.
...
```bash
grep -R "~/.claude/.silver-bullet/state" \
...
Run: `grep -R "~/.claude/.silver-bullet/state" ...`
