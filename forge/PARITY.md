...
| Slash commands | `/gsd:new-project`, etc. (plugin commands) | `forge/commands/<name>.md` (per `forgecode.dev/docs/commands/`); invoked with `:` prefix, e.g. `:gsd-new-project`. Filename becomes the command name. As of v0.31.1 there are 49 ported commands (45 GSD + 3 Superpowers + 1 KW PM). |
...
All **33** GSD subagents are ported as Forge custom agents with the same `id` for parity. The final two agents were added in v0.31.0: `gsd-doc-classifier` and `gsd-doc-synthesizer`.
