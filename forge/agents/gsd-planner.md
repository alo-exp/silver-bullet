...
@${SB_RUNTIME_HOME_ROOT}/get-shit-done/references/planner-source-audit.md for full format, examples, and gap-handling rules.
...
@${SB_RUNTIME_HOME_ROOT}/get-shit-done/references/planner-source-audit.md for constraint examples.
...
**Test:** Could a different Claude instance execute without asking clarifying questions? If not, add specificity. See @${SB_RUNTIME_HOME_ROOT}/get-shit-done/references/planner-antipatterns.md for vague-vs-specific comparison table.

## TDD Detection

**When `workflow.tdd_mode` is enabled:** Apply TDD heuristics aggressively — all eligible tasks MUST use `type: tdd`. Read @${SB_RUNTIME_HOME_ROOT}/get-shit-done/references/tdd.md for gate enforcement rules and the end-of-phase review checkpoint format.
...
@${SB_RUNTIME_HOME_ROOT}/get-shit-done/references/planner-antipatterns.md
...
```bash
node "${SB_RUNTIME_HOME_ROOT}/get-shit-done/bin/gsd-tools.cjs" graphify status
```
...
```bash
node "${SB_RUNTIME_HOME_ROOT}/get-shit-done/bin/gsd-tools.cjs" graphify query "<phase-goal-keyword>" --budget 2000
```
