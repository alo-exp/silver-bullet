...
If this skill is running from a native skill mirror such as `$HOME/.cursor/skills/silver-quality-gates/SKILL.md` and `${PLUGIN_ROOT}/skills/modularity/SKILL.md` does not exist, resolve the hidden packaged Cursor source root in this order:

```bash
for candidate in \
  "$HOME/.cursor/plugins/cache/alo-labs/silver-bullet/current/skill-source" \
  "$(find "$HOME/.cursor/plugins/cache/alo-labs/silver-bullet" -mindepth 2 -maxdepth 2 -type d -path '*/skill-source' 2>/dev/null | sort -V | tail -n 1)"; do
