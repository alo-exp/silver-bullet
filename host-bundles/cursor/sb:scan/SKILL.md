...
- scans `docs/sessions/*.md` plus any relevant Cursor, primary host, or SB-maintained session/handoff logs it can safely identify
- supports `--agent-env auto|Cursor|primary host|kay` to select the runtime session store family explicitly
...
1. Run `scripts/silver-scan.sh scan --mode <interactive|autonomous> --agent-env <auto|Cursor|primary host|kay> --format json`.
