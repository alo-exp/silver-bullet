...
| "delegate to claude", "run in claude", "claude subagent", "claude tui", "have claude implement", host supervises Claude executes | `silver:agent-claude` | On-demand `AF-AGENT-DELEGATE` — host briefs and supervises native `AGENT-DELEGATE` worker; external Claude TUI executes (V2 default-on; `SB_AGENT_DELEGATE_V2=0` rollback) |
