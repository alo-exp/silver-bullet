...
- `silver-bullet@alo-labs` (Cursor marketplace)
...
# Cursor host: ./scripts/install-<runtime>.sh --purge-legacy-skills
```
...
```bash
REG="$HOME/.cursor/plugins/installed_plugins.json"
...
Check whether `$HOME/.cursor/plugins/cache/silver-bullet/silver-bullet/` exists. If it does, remove it:

```bash
if [[ -z "$HOME" ]]; then
  echo "WARNING: HOME is unset — skipping stale cache cleanup."
else
  STALE_CACHE="$HOME/.cursor/plugins/cache/silver-bullet/silver-bullet"
...
Do NOT remove `$HOME/.cursor/plugins/cache/silver-bullet/alo-labs/` — that is the newly installed version.
...
`primary host`, `Cursor`, or `task host` via `SILVER_BULLET_RUNTIME`, `SB_PLUGIN_ROOT`, or host env vars).
...
2. Pre-index skill registration (upstream Install Step 2):
   ...
   - **Cursor:** `graphify install --project --platform <runtime>`
   - **task host:** skip
...
4. Post-index always-on (upstream "Make your assistant always use the graph"):
   ...
   - **Cursor:** `graphify host install (see install guide) --project`
...
3. Pre-index (Cursor): `host plugin marketplace (see install guide) add rohitg00/agentmemory`; `Cursor plugin add agentmemory@agentmemory`
4. Post-index MCP connect:
   - **primary host:** `agentmemory connect (see install guide)`
   - **Cursor:** `agentmemory connect (see install guide) --with-hooks`
