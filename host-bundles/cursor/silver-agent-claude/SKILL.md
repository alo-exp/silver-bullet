...
**Contrast with `/silver:agent-cursor`:** Cursor is the other on-demand host-delegation sibling. Use **agent-claude** when Cursor TUI is the intended executor; use **agent-cursor** for Cursor CLI. **`/silver:agent-codex` was intentionally removed** from this branch — do not restore or reference it as a sibling.
