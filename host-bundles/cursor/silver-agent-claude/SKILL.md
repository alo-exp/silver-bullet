...
**Contrast with `/silver:agent-cursor`:** Cursor is the other on-demand host-delegation sibling. Use **agent-claude** when Claude Code TUI is the intended executor; use **agent-cursor** for Cursor CLI. **`/silver:agent-codex` was intentionally removed** from this branch — do not restore or reference it as a sibling.
...
- Proxy/MiniMax hosts: document `$HOME/.cursor/settings.json` env; delegate inherits caller auth (not `env -i` clean-env).
...
| **E2E-105** | Fresh `CLAUDE_CONFIG_DIR` + `$HOME/.cursor/.silver-bullet` per delegation wave in lightweight mode |
