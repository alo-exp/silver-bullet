...
5a. Run step 3.7.5 to re-register or refresh SB hooks in `$HOME/.cursor/settings.json`.
...
**If NO existing project instruction file**: do not create a new agent instruction file during Cursor initialization. Silver Bullet's own instructions live in `silver-bullet.md`; a project instruction file is optional.
...
Applies only when an existing project instruction file was found in step 3.1b. If none was found, skip this step — Silver Bullet does not synthesize a new project instruction file during Cursor init.
...
### 3.4.2 RTK awareness (Cursor / optional)
...
import json, os, sys
reg = os.path.expanduser('$HOME/.cursor/plugins/installed_plugins.json')
with open(reg) as f:
    data = json.load(f)
...
if [[ "${SILVER_BULLET_RUNTIME:-}" == "task host" ]]; then
  python3 "${PLUGIN_ROOT}/scripts/lib/install-<runtime>/merge-cursor-hooks.py" "$INSTALL_PATH"
else
  python3 "${PLUGIN_ROOT}/scripts/lib/install-<runtime>/merge-cursor-hooks.py" "$INSTALL_PATH"
fi
```

`scripts/lib/install-<runtime>/merge-cursor-hooks.py` substitutes the plugin install path, removes stale mirrored Silver Bullet hook registrations from other app roots or placeholder entries, and appends only new hook entries — never duplicates. On nonzero exit, warn but do NOT stop init:
