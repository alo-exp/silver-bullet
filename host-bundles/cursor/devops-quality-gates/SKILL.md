...
description: This skill should be used to apply 7 IaC-adapted quality dimensions against infrastructure and DevOps changes. Use after /sb:blast-radius and before /sb:plan in the devops-cycle workflow. Skips usability because IaC has no direct user interface. All dimensions must pass — any ❌ is a hard stop.
...
proceeds to `sb:plan`. A ❌ is a hard stop — redesign before continuing.
...
**Dimension skills root**: Set `DIMENSION_SKILLS_ROOT="${PLUGIN_ROOT}/skills"` by default. If this skill is running from a native skill mirror such as `$HOME/.cursor/skills/devops-quality-gates/SKILL.md` and `${PLUGIN_ROOT}/skills/modularity/SKILL.md` does not exist, use the hidden packaged Cursor source root instead:

```bash
DIMENSION_SKILLS_ROOT="$HOME/.cursor/plugins/cache/alo-labs/silver-bullet/current/skill-source"
```
...
- If **all applicable dimensions pass** → output "DevOps quality gates passed. Proceed to `sb:plan`."
- If **any dimension fails** → output "DevOps quality gates FAILED. Redesign required before planning."
  ...
  Do NOT proceed to `sb:plan` until all failures are resolved and this skill is re-run.
