...
# /sb:validate — Pre-Build Gap Analysis
...
**Plan-only mode:** `.planning/SPEC.md` is absent but at least one `*PLAN.md` exists under `.planning/phases/`. Use this for `sb:devops`, `sb:fast` Tier 2, and other infra-only work where `workflow-chain-guard` requires a `sb:validate` marker before implementation edits but no product SPEC exists yet. Skip Step 1 and Step 3a (AC coverage); run Steps 2, 3b–3d, and 3f only.
...
1. Verify `.planning/SPEC.md` exists:
   ...
   FINDING [BLOCK] VAL-000: Neither SPEC.md nor PLAN.md found — nothing to validate
     Spec ref: pre-flight
     Plan ref: missing
     Resolution: Run /sb:plan (or /sb:spec for product work) before /sb:validate
   ```
...
>
> A. Return to {/sb:spec in spec mode | /sb:plan in plan-only mode} to resolve the gaps
> B. Show me the BLOCK findings again
> C. I have resolved the issues — re-run /sb:validate
...
> B. Return to {/sb:spec in spec mode | /sb:plan in plan-only mode} to address WARN findings first
...
Generated by /sb:validate on {date}. Consumed by pr-traceability.sh for PR description.
