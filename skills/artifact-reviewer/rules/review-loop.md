...
invoke /sb:completion-audit or /sb:verify as appropriate for the artifact
...
- After the loop completes, `sb:completion-audit` or `sb:verify` MUST be invoked before the producing step is marked done — no completion claim without fresh verification evidence
...
```
${SB_RUNTIME_HOME_ROOT}/.silver-bullet/review-state/{artifact-hash}.json
```
...
```bash
# State directory
SB_REVIEW_STATE="${SB_RUNTIME_HOME_ROOT}/.silver-bullet/review-state"
...
- **On milestone completion** (`sb:release` milestone archive): archive the current REVIEW-ROUNDS.md to `.planning/archive/{milestone-slug}/REVIEW-ROUNDS.md` and start a fresh empty file
