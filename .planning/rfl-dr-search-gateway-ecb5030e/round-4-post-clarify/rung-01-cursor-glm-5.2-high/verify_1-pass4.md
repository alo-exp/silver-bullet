...
**Phase:** VERIFY-ONLY pass 1/2 (`rung_1_verify_1`, post ACCEPT-apply of S1 / I-16)  
...
**Graphify:** `graphify query "allow-private stable_hash fingerprint q3" --budget 2000` (128 nodes; oriented S1/M-4 fingerprint contract)  
**Apply ref:** S1 / I-16 (`--allow-private` in `stable_hash`) · **Prior verify:** [verify_1-pass3.md](verify_1-pass3.md)

## Verdict

**VERIFY_PASS** — S1 / I-16 APPLY confirmed at pinned SHA; all fingerprint-contract checks present with file:line evidence; Facebook `must_search: false` intact; I-1–I-15 remain present.
...
|-------|--------|-----------------|
| `--allow-private` IS a `stable_hash` boolean field (default false) | PASS | L84 (M-4): `` `--allow-private` IS a `stable_hash` field **(boolean; default false)** ``; L458 §6.3: `` **`--allow-private` IS in the hash** (boolean field; default false; round-4 pass 4 S1) `` |
...
| §8.4 updated (`src/cache.rs` fingerprint list) | PASS | L771 §8.4 item 2: `` `q3_` fingerprint includes `-p`+canonicalized domains+filters+`--allow-private` (**not** count, **not** TTL, **not** `--max-chars` …) `` |
| §4.1 updated | PASS | L330 §4.1: `` fingerprint that includes providers+domains+filters + `--allow-private` boolean (**not** `count`, **not** TTL, **not** `--max-chars` …) `` |
| §6.3 updated (full `stable_hash` field contract) | PASS | L432 `### 6.3 Cache — layout, fingerprint, TTL, multi-process`; L458 full field list with `--allow-private` in hash |
| Historical M-4 ledger updated (supersedes 2026-08-18 “not in hash” for this flag only) | PASS | L84 (missing High+ item 10 M-4): `` **supersedes** the 2026-08-18 “not in hash” clause for this flag only `` |
| Round-4 post-clarify ACCEPT bullet (I-16) | PASS | L85: `` `--allow-private` is a `stable_hash` boolean (human writer must not poison fleet `q3_` hits) `` |

## Facebook + I-1–I-15 regression guard

| Check | Status | Evidence (plan) |
|-------|--------|-----------------|
| Facebook `must_search: false` | PASS | L54: `` Facebook stays **not** must-search ``; L198: `` Facebook: `must_search: false` ``; L339 §4.3 test matrix: `` facebook `must_search=false` `` |
| I-1 X dedup in SB orchestrator | PASS | L85 ACCEPT; L101 §1.4 dedup in `search_orchestrator.py` |
| I-2 `site:` rows require Serper/Brave consent | PASS | L85 ACCEPT; L219 §2.7 `site:` dependency |
...
- Parent may proceed to `verify_2` pass 4 + orchestrator greps.
