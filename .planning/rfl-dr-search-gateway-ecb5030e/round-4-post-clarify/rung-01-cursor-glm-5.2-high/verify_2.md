...
**Phase:** VERIFY-ONLY pass 2/2 (`rung_1_verify_2`)  
...
**VERIFY_PASS** — independent re-read confirms all 13 round-4 post-clarify rung-1 ACCEPTs (F1–F13) remain present at the pinned SHA with explicit, quotable evidence. No leftover defects vs the APPLY charter.

## F1–F13 evidence (independent)

| ID | Charter item | Status | Evidence (plan) |
|----|--------------|--------|-----------------|
| F1 | X dedup in SB orchestrator | PASS | L101 §1.4: "**Dedup (SB orchestrator, locked):** … Dedup lives in **`search_orchestrator.py`**, not the fork. Key: prefer tweet/status id; else canonical `x.com` / `twitter.com` status URL." L178 §2.5: "Dedup is the SB orchestrator contract in §1.4 (not the fork)." |
...
| F4 | Non-Cursor: URLs + `search config set` + consent file | PASS | L90 §1.3: "prints manual URLs (the same obtain URLs as §2.8) and tells the user to run `search config set keys.<name> -` themselves. The agent still writes `consented_channels` … `providers_configured` is set only after `search doctor` / `agent-info` sees the key." L220 §2.7 step 4 echoes print-URL path for non-Cursor hosts. |
| F5 | One X row; list `provider`/`bucket` | PASS | L120 §2.2: "`provider` (string or list …) `bucket` (string or list …) **X** is one catalog row with list-valued `provider`/`bucket` (`[x, xweb]`)." L178 §2.5: "**Catalog encoding (locked):** one X row; … `provider: [x, xweb]` and `bucket: [x, xweb]`." L339 §4.3 test: "xweb bucket/provider present on the **one** X row as list-valued `provider`/`bucket` including `x` and `xweb`." |
...
| F7 | No binary fallback | PASS | L324 §3.4: "**No binary fallback (Phase 1):** if `alo-exp/search-cli` / tag `v0.9.0-sb.1` is unavailable … `cargo install` fails; there is no cached-binary / mirror path. Record as install gap." |
...
| F9 | PAT/secret rotation alerts | PASS | L344 §4.4: alerts include "GitHub/GitLab PAT expiry, Stack Exchange key rotation, Reddit OAuth app-secret rotation, Brave remaining if configured." |
...
| F13 | Metrics = usage + run_manifest | PASS | L344 §4.4: "**Metrics (Phase 6 docs):** `search usage --json` plus `run_manifest` shards; no required per-channel latency/cache-hit/bucket-wait export in Phase 1 …" |
...
- This verify pass is independent of `verify_1.md`; both passes agree VERIFY_PASS.
- Verify is not a Policy F streak; streaks are CLEAN **reviews** only.
