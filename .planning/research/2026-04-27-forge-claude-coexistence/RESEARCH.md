...
|---|---|---|---|---|---|
| 1 | Skill files | `${SB_RUNTIME_HOME_ROOT}/skills/`, `${SB_RUNTIME_HOME_ROOT}/plugins/cache/.../skills/` | `~/forge/skills/`, `.forge/skills/` | Disjoint paths | None |
| 2 | Agent files | `${SB_RUNTIME_HOME_ROOT}/agents/` | `~/forge/agents/`, `.forge/agents/` | Disjoint paths | None |
...
| 7 | **State file** `${SB_RUNTIME_HOME_ROOT}/.silver-bullet/state` | Reads + writes (one skill per line) via `record-skill.sh` | **Doesn't read or write** | **Mismatch** | **HIGH** |
| 8 | **Trivial bypass** `${SB_RUNTIME_HOME_ROOT}/.silver-bullet/trivial` | Created at SessionStart, removed on first non-trivial Edit | **Doesn't read or write** | One-way: Forge-applied edits don't remove Claude's bypass file | MEDIUM |
| 9 | **Branch tracker** `${SB_RUNTIME_HOME_ROOT}/.silver-bullet/branch` | Used to detect branch switches → reset state | **Doesn't read or write** | One-way: Forge work on a different branch doesn't reset Claude's state on switch back | LOW |
...
| 14 | `dev-cycle-check.sh` | Blocks edits to `${SB_RUNTIME_HOME_ROOT}/plugins/cache/**` | N/A in Forge | Forge users could legitimately edit `~/forge/skills/` from a Forge session — but `dev-cycle-check.sh` only fires in Claude Code | None |
...
2. Both write skill applications to `${SB_RUNTIME_HOME_ROOT}/.silver-bullet/state` — atomically appended (one line per skill, deduped via `grep -qx` before write)
...
T7  hook reads ${SB_RUNTIME_HOME_ROOT}/.silver-bullet/state — NO skills tracked from this branch
...
**Mechanism:** Each Forge custom agent that corresponds to an SB skill (or each invocation by the main Forge agent of an SB skill via auto-application) appends the skill name to `${SB_RUNTIME_HOME_ROOT}/.silver-bullet/state` if the file exists.

**Where to add:**
- A new shared bash helper, `forge/scripts/record-skill-on-claude.sh`, that appends `<skill-name>` to `${SB_RUNTIME_HOME_ROOT}/.silver-bullet/state` (with `grep -qx` dedup, same as `record-skill.sh`).
...
**Cons:**
- Couples Forge to Claude's storage path (`${SB_RUNTIME_HOME_ROOT}/.silver-bullet/state`). If Claude is not installed, the file simply doesn't exist and the marker write is a no-op (`mkdir -p` + `touch` + append guard).
- Requires every parent SB skill to remember to call the recorder. Easy to forget.
- Violates a user-stated rule: "Never touch trivial bypass or Silver Bullet state files. STRICTLY never touch ${SB_RUNTIME_HOME_ROOT}/.silver-bullet/trivial or any SB state file; invoke required skills instead." — but that rule is about *agent rationalization*, not about a system-level cooperation API. v0.29.0 should formalize the API as legitimate cooperation, not bypass.
...
- **A** for the technical fix: Forge agents cooperatively write to `${SB_RUNTIME_HOME_ROOT}/.silver-bullet/state` via a shared `forge-record-skill` helper agent. Bounded scope, minimal Claude-SB changes.
...
1. Create `forge/agents/forge-record-skill.md` — a new custom agent with `id: forge-record-skill`, `description: "Records that an SB skill was applied. Appends skill name to ${SB_RUNTIME_HOME_ROOT}/.silver-bullet/state if the file/directory exists, no-op otherwise. Cooperates with Claude-SB's completion-audit and stop-check hooks."`, `tools: [shell]`, `tool_supported: true`, `temperature: 0.0`.
2. Body of the agent: `mkdir -p ${SB_RUNTIME_HOME_ROOT}/.silver-bullet 2>/dev/null && grep -qx "<skill-id>" ${SB_RUNTIME_HOME_ROOT}/.silver-bullet/state 2>/dev/null || echo "<skill-id>" >> ${SB_RUNTIME_HOME_ROOT}/.silver-bullet/state` (with proper guard for `<skill-id>` parameter passing).
...
3. **Do we also write to `${SB_RUNTIME_HOME_ROOT}/.silver-bullet/branch`?** Forge changes branches via shell tool. Should it touch the branch file so Claude's next session-start handles the switch correctly? Probably yes for symmetry.

4. **Trivial-bypass cooperation?** Forge has no concept of "trivial session." If a Forge session edits source files in a project where Claude has the trivial bypass active, should Forge remove the bypass file? Probably yes — Forge's `forge-session-init` agent is a natural place to call `rm -f ${SB_RUNTIME_HOME_ROOT}/.silver-bullet/trivial` after the first Edit/Write, mirroring `hooks/lib/trivial-bypass.sh` removal behavior.
...
The original Option A (Forge cooperatively writes to `${SB_RUNTIME_HOME_ROOT}/.silver-bullet/state` so its skill applications are visible to Claude-SB hooks) **is no longer the right primitive.** With per-phase ownership:
