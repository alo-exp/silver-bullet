...
Prior verify_1: `rung-03-opencode-go-qwen3.8-max/verify-1.md` — CLEAN / VERIFY_PASS, **Grok 4.6 High substitute** after Pi 401×2. Do **not** copy its verdict blindly; re-audit independently. Do not treat verify-1 as sufficient.

Parent Policy A already applied:
- **F-1 MED REJECT** — do not expect `--` TOC/body hrefs. GFM = strip punct then collapse whitespace to a **single** hyphen. `ws0--ws0b` must remain 0. Do not treat Qwen `--` table as leftover.
...
4. verify-2.md MUST include: SHA-256 each copy + sizes + byte-identical; Prior ACCEPT HOLD/leftover table (F-2 HOLD; F-1 REJECT held — no `--`); remaining findings; Verdict CLEAN or NOT CLEAN; leftovers none-or-list; SHA; EXIT; VERIFY_PASS or VERIFY_FAIL.

FORBIDDEN: freeze Edit/Write, clarify, AskQuestion, combining with verify_1, remapping models.