#!/usr/bin/env bash
# Bounded Pi invoke for rung_08_verify_1. Hang-kill ~6m if idle (0% CPU / no events / no verify-1.md).
set -u
...
START="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
{
  ...
  echo "phase=rung_08_verify_1 attempt=${ATTEMPT}"
} >"$OUT"
: >"$ERR"
...
while kill -0 "$INV_PID" 2>/dev/null; do
  ...
  if [[ "$progress" -eq 0 && "$elapsed" -ge "$HANG_SEC" ]]; then
    ...
    # also kill descendants named pi
