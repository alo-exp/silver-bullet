#!/usr/bin/env bash
# Fix-retry after Claude NI write-first rails. Exact parent invoke recipe.
# Do not export PI_NI_ZERO_BYTE_IDLE_SEC. Do not overwrite prior hang logs.
set -u
...
LOGDIR="${RUNG}/logs"
OUT="${LOGDIR}/review-fix-retry-stdout.txt"
ERR="${LOGDIR}/review-fix-retry-stderr.txt"
mkdir -p "$LOGDIR"
if [[ -f "$REPORT" ]]; then
  ts="$(date -u +%Y%m%dT%H%M%SZ)"
  mv "$REPORT" "${LOGDIR}/review.md.stale-${ts}"
  echo "archived stale review.md -> ${LOGDIR}/review.md.stale-${ts}"
fi
...
START="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
{
  ...
  echo "phase=rung_10_review attempt=fix-retry"
  echo "work-dir=${RUNG}"
  echo "rails=thinking-off,no-context-files,no-skills,tools=read,bash,edit,write"
} >"$OUT"
: >"$ERR"
bash "$INVOKE" \
  --work-dir "$RUNG" \
