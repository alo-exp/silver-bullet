...
Write the official report to `./review.md` in this work directory (same file as the absolute path in Tasks item 3). Do **not** reply with a plan-only sentence. Do **not** exit after hashing/planning. Do **not** leave an IN_PROGRESS stub. The official deliverable is a written file. Completion requires `./review.md` on disk (≥2500 bytes) with findings + counts + CLEAN/NOT CLEAN. Stdout-only hash/plan text is incomplete.
