...
Record the SHA you actually hashed. Do **not** use stale SHA `07b98609…` / 620985, `d5343ac1…` / 621095, or `edff7c0c…` / 621101 as current. Do **not** cite `d5343ac1` or `edff7c0c` as the current freeze.
...
- Closed / do not reopen as product forks: KEEP REJECT / Q1–Q3 / Part A then Part B; YAML 33/33 remain pending; F-1 (Qwen double-hyphen GFM) REJECT; F-2 HOLD at L3246 `#### \`blocked_advisor_state\` (row 14)` (twin ~L3052).
...
- Verification signals: sha256 both copies must be `4c18af57…` / 621233; 33 pending todos; forbid-only multi-ai-task and agent-wrap; FAST not a Job; one mermaid; closed KEEP REJECT / Q1–Q3 / Part A then Part B.
...
- Treating empty logs/work, a Grok substitute review, or a missing `review.md` as completing this rung
