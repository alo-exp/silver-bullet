...
This is `/silver:review-fix-ladder` only. Do **not** run `/silver:clarify`. Do not AskQuestion. Do not encode product forks. Do **not** copy `review.md` or `verify-1.md` — re-derive every check from disk independently.
...
Independent checks (re-run from disk; do not copy review.md or verify-1.md):
...
5. Write ONLY `./verify-2.md`. Print the same verdict on stdout after the file exists. Then stop.
...
FORBIDDEN: freeze edits, YAML execute, git branch switch, clarify, AskQuestion, combining verify passes, copying review.md, copying verify-1.md, Grok-substitute, starting rung 3.
