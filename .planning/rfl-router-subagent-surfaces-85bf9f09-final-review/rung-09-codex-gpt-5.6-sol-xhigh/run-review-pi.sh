#!/usr/bin/env bash
# Bounded Pi invoke for rung_09_review. Hang-kill ~4m if idle (0% CPU / no events / no review.md).
...
START="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
{
  ...
  echo "phase=rung_09_review attempt=${ATTEMPT}"
} >"$OUT"
: >"$ERR"
...
# Bounded idle: a few minutes, not 13m (rung 8 review hung).
HANG_SEC=240
POLL=15
elapsed=0
progress=0
