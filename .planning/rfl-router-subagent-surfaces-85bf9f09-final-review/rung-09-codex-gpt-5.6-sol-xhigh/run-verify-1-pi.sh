#!/usr/bin/env bash
# Bounded Pi invoke for rung_09_verify_1. Hang-kill ~4m if idle (0% CPU / no events / no verify-1.md).
# Avoid bash 3.2 + set -u empty-array EXTRA[@] unbound.
set -u
RUNG="/Users/shafqat/projects/silver-bullet/repo/.planning/rfl-router-subagent-surfaces-85bf9f09-final-review/rung-09-codex-gpt-5.6-sol-xhigh"
SB_ROOT="/Users/shafqat/projects/silver-bullet/repo"
INVOKE="${SB_ROOT}/scripts/agent-pi/invoke.sh"
REPORT="${RUNG}/verify-1.md"
WORK="${RUNG}/work"
LOGDIR="${RUNG}/logs"
ATTEMPT="${1:-1}"
RETRY_FLAGS=""
if [[ "$ATTEMPT" == "1" ]]; then
  OUT="${LOGDIR}/verify-1-stdout.txt"
  ERR="${LOGDIR}/verify-1-stderr.txt"
else
...
{
  echo "START ${START}"
  echo "PI_PROVIDER=omniroute PI_MODEL=codex/gpt-5.6-sol-xhigh"
  echo "phase=rung_09_verify_1 attempt=${ATTEMPT}"
} >"$OUT"
: >"$ERR"

export PI_PROVIDER=omniroute
export PI_MODEL=codex/gpt-5.6-sol-xhigh
...
# Bounded idle: ~4m, matching review (rung 9 review hung EXIT 143).
HANG_SEC=240
POLL=15
elapsed=0
progress=0
while kill -0 "$INV_PID" 2>/dev/null; do
  ...
  if [[ "$progress" -eq 0 && "$elapsed" -ge "$HANG_SEC" ]]; then
    echo "HANG_KILLED after ~$((elapsed/60))m: pi idle, no events.jsonl, no verify-1.md" >>"$OUT"
