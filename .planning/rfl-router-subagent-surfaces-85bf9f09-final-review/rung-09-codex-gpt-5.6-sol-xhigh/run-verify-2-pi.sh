#!/usr/bin/env bash
# Bounded Pi invoke for rung_09_verify_2. Hang-kill ~4m if idle (0% CPU / no events / no verify-2.md).
...
START="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
{
  ...
  echo "phase=rung_09_verify_2 attempt=${ATTEMPT}"
} >"$OUT"
: >"$ERR"
...
# Bounded idle: ~4m (same as rung 9 review hang-kill and verify_1 wrapper).
HANG_SEC=240
POLL=15
elapsed=0
progress=0
