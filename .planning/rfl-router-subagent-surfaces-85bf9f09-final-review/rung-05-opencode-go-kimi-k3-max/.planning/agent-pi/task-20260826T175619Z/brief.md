You are on rung 5/11: model=opencode-go/kimi-k3-max (OpenCode Go Kimi K3 Max via `/silver:agent-pi` / OmniRoute). User-named Kimi — do **not** remap to Grok. Never Fast.
...
Write the official report to `./review.md` in this work directory (same file as the absolute path in Tasks item 3). Do not edit either freeze copy. Do not copy `review-prior-wave.md`. Existing `review.md` is a prior-wave Grok substitute of stale SHA `d5343ac1…` / 621095 — overwrite it with a fresh official Pi Kimi review of `edff7c0c…` / 621101.
