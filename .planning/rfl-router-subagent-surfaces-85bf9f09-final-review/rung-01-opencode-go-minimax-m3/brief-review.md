You are on rung 1/11: model=opencode-go/minimax-m3, reasoning=host-default.
Phase: REVIEW-ONLY (rung_01_review)
...
3. You MUST write the full report with a write tool to `./review.md` in this work directory (the rung folder). A plan-only sentence, assistant-only reply, or "I will write review.md" without the file is a failed task. Do not copy `review-grok-substitute.md`. Official `./review.md` must be Pi MiniMax this run.
