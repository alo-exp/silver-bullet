#!/usr/bin/env bash
# Poll Round Codex-3 REAL Tier B smoke driver until exit; rescore rows 1,3,6.
set -euo pipefail
...
INTERVAL="${2:-75}"
POLL_LOG="${ROOT}/.planning/enterprise-e2e/.codex-r3-tierb-poll.log"
RESCORE_LOG="${ROOT}/.planning/enterprise-e2e/.codex-r3-tierb-rescore.log"
MATRIX_LOG="${ROOT}/.e2e-matrix-codex-live.log"
LEDGER="${ROOT}/.planning/enterprise-e2e/ROUND-CODEX-3-LEDGER.md"
CHECKPOINT="${ROOT}/.planning/enterprise-e2e/.codex-r3-tierb-checkpoint.md"
...
# pipefail-safe: pgrep/grep exit 1 when no match — must not kill tmux poll pane
...
{
  printf '# Round Codex-3 REAL Tier B checkpoint\n\n'
  printf -- '- **When:** %s *(EXIT)*\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  printf -- '- **Branch:** enterprise-e2e/codex @ %s\n' "$(git rev-parse --short HEAD 2>/dev/null || echo unknown)"
  printf -- '- **Driver PID:** %s EXITED\n' "$DRIVER_PID"
  printf -- '- **Tier B rescore:** %s/3 — [.codex-r3-tierb-rescore.log](./.codex-r3-tierb-rescore.log)\n' "$pass_count"
} >"$CHECKPOINT"

{
  printf '\n### Poll checkpoint %s (Round Codex-3 REAL Tier B exit)\n\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  printf '| Field | Value |\n|-------|-------|\n'
