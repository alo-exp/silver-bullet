#!/usr/bin/env bash
# Poll codex-r3-force1416 driver until exit; frozen-merge rescore from force141619 @ dead1460.
# One-pass: 19 PASS rows frozen (incl row 19); live rescore rows 14,15,16; rows 21–22 internal inherit 3/4.
set -euo pipefail
...
RESCORE_LOG="${ROOT}/.planning/enterprise-e2e/.codex-r3-force1416-rescore.log"
FROZEN_RESCORE_LOG="${ROOT}/.planning/enterprise-e2e/.codex-r3-force141619-rescore.log"
FROZEN_BASELINE_SHA="${SB_E2E_FROZEN_BASELINE_SHA:-dead1460}"
...
CHECKPOINT="${ROOT}/.planning/enterprise-e2e/.codex-r3-force1416-checkpoint.md"
FORCE_ROWS=(14 15 16)
export SB_E2E_REVIEW_LADDER_LEDGER="${SB_E2E_REVIEW_LADDER_LEDGER:-$ROOT/.planning/enterprise-e2e/ROUND-3-LEDGER.md}"
...
run_frozen_merge_rescore() {
  ...
  for row in $(seq 1 20); do
    if ! is_force_row "$row" && frozen_row_passes "$row"; then
      if grep -qE "^row ${row} PASS \\(frozen@" "$FROZEN_RESCORE_LOG" 2>/dev/null; then
        frozen_tag="$(grep -E "^row ${row} PASS \\(frozen@" "$FROZEN_RESCORE_LOG" | sed -E 's/^row [0-9]+ PASS \(frozen@([^)]+)\).*/\1/')"
        printf 'row %d PASS (frozen@%s)\n' "$row" "$frozen_tag" >>"$RESCORE_LOG"
      else
        printf 'row %d PASS (frozen@force141619)\n' "$row" >>"$RESCORE_LOG"
      fi
      pass=$((pass + 1))
      continue
    fi
    ...
    if enterprise_e2e_row_requires_product_commit "$row"; then
      if ! enterprise_e2e_assert_row_product_commit_rescore "$row" "$WORK_DIR" >>"$RESCORE_LOG" 2>&1; then
        if enterprise_e2e_row_outcome_only_rerun "$row"; then
          printf 'row %d §5b outcome-only (product frozen)\n' "$row" >>"$RESCORE_LOG"
        else
...
draft_product_audit() {
  ...
  local commit_count fixture_sha sb_sha
  ...
  commit_count="$(git -C "$fixture" rev-list --count "${baseline}..HEAD" 2>/dev/null || echo 0)"
  {
    ...
    printf '**Status:** DRAFT — auto-generated on force1416 poll-exit @ %s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
    ...
    printf '**Rescore:** [.codex-r3-force1416-rescore.log](./.codex-r3-force1416-rescore.log)\n'
    printf '**Frozen merge:** [.codex-r3-force141619-rescore.log](./.codex-r3-force141619-rescore.log) @ `%s`\n\n' "$FROZEN_BASELINE_SHA"
    printf '## Summary\n\n'
    ...
    printf '- **Matrix rescore:** see force1416 rescore log for row-level truth\n'
...
{
  printf '# codex-r3-force1416 checkpoint\n\n'
  printf -- '- **When:** %s *(EXIT)*\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  printf -- '- **Branch:** enterprise-e2e/codex @ %s\n' "$(git rev-parse --short HEAD 2>/dev/null || echo unknown)"
  printf -- '- **Frozen baseline:** %s — 19 PASS rows frozen @ force141619\n' "$FROZEN_BASELINE_SHA"
  printf -- '- **Driver PID:** %s EXITED\n' "$DRIVER_PID"
  printf -- '- **Rescore:** %s/22 — [.codex-r3-force1416-rescore.log](./.codex-r3-force1416-rescore.log)\n' "$pass_count"
  printf -- '- **Poll log:** [.codex-r3-force1416-poll.log](./.codex-r3-force1416-poll.log)\n'
} >"$CHECKPOINT"

{
  printf '\n### Poll checkpoint %s (force1416 exit @ %s)\n\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$(git rev-parse --short HEAD 2>/dev/null || echo unknown)"
  printf '| Field | Value |\n|-------|-------|\n'
  printf '| **Policy** | One pass — frozen **19 PASS** @ force141619; FORCE **14,15,16** live |\n'
