#!/usr/bin/env bash
# R9 sequential pilots: row 3 then row 11 (honest TTY via caller tmux).
set -euo pipefail
MAIN="/Users/shafqat/projects/silver-bullet/repo"
SB_ROOT="/private/tmp/sb-main-row11-fp"
LOG="${MAIN}/.e2e-r9-pilot-row3-then-11-live.log"
PIDFILE="${MAIN}/.e2e-r9-pilot-row3-then-11.pid"
...
# Sync harness patches from MAIN (uncommitted prompt tweaks) into isolated SB_ROOT.
rsync -a "$MAIN/tests/e2e-live/lib/skill-prompt.sh" "$SB_ROOT/tests/e2e-live/lib/skill-prompt.sh"
rsync -a "$MAIN/scripts/enterprise-e2e/matrix.sh" "$SB_ROOT/scripts/enterprise-e2e/matrix.sh"
...
# Fresh hook findings for OUT-HOOK-01 / OUT-HEAL-01 (ignore stale row blockers).
_findings="${SB_ROOT}/.e2e-tui-watch-findings.jsonl"
if [[ -f "$_findings" ]]; then
  cp "$_findings" "${_findings}.bak-$(date -u +%Y%m%dT%H%M%SZ)"
  : >"$_findings"
fi
...
export SB_E2E_SURFACE_SKIP=0 SB_E2E_MATRIX_FORCE=1 SB_E2E_MATRIX_FORCE_ALL=1 SB_E2E_ISOLATED_CLAUDE_CONFIG=1
...
exec >>"$LOG" 2>&1
echo "=== R9 pilot rows 3→11 $(date -u +%Y-%m-%dT%H:%M:%SZ) pid=$$ sb=$(git -C "$SB_ROOT" rev-parse --short HEAD) main=$(git -C "$MAIN" rev-parse --short HEAD) cumulative=1 ==="
cd "$SB_ROOT"
...
row_passed() {
  ...
  slice="$(awk -v n="$n" '
    /^=== Row [0-9]+:/ {
      rid = $0; sub(/^=== Row /, "", rid); sub(/:.*$/, "", rid)
      if (rid == n) { buf = ""; collecting = 1 } else if (collecting) { collecting = 0 }
      next
    }
    /^=== PILOT row/ { if (collecting) collecting = 0; next }
    collecting { buf = buf $0 ORS }
    END { printf "%s", buf }
  ' "$LOG" 2>/dev/null || true)"
  printf '%s' "$slice" | grep -q "  FAIL:" && return 1
  printf '%s' "$slice" | grep -q "OUTCOMES: all applicable criteria pass" && return 0
  printf '%s' "$slice" | grep -q "  PASS: evidence at" && return 0
  return 1
}
