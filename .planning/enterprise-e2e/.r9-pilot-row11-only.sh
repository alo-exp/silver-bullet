...
PIDFILE="${MAIN}/.e2e-r9-pilot-rows-6-11.pid"
...
echo "=== R9 pilot row 11 only $(date -u +%Y-%m-%dT%H:%M:%SZ) pid=$$ ===" >>"$LOG"
exec >>"$LOG" 2>&1
echo "=== R9 pilot row 11 only $(date -u +%Y-%m-%dT%H:%M:%SZ) pid=$$ commit=$(git -C "$MAIN" rev-parse --short HEAD) ==="
cd "$SB_ROOT"
