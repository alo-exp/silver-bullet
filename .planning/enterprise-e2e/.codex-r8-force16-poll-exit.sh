#!/usr/bin/env bash
# Poll codex-r8-force16 driver until exit; full rescore + ledger + Phase C if 22/22.
set -euo pipefail
...
INTERVAL="${2:-75}"
POLL_LOG="${ROOT}/.planning/enterprise-e2e/.codex-r8-force16-poll.log"
RESCORE_LOG="${ROOT}/.planning/enterprise-e2e/.codex-r8-force16-rescore.log"
MATRIX_LOG="${ROOT}/.e2e-matrix-codex-live.log"
LEDGER="${ROOT}/.planning/enterprise-e2e/ROUND-CODEX-1-LEDGER.md"
CHECKPOINT="${ROOT}/.planning/enterprise-e2e/.codex-r8-force16-checkpoint.md"
...
{
  printf '# codex-r8-force16 checkpoint\n\n'
  printf -- '- **When:** %s *(EXIT)*\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  printf -- '- **Branch:** enterprise-e2e/codex @ %s\n' "$(git rev-parse --short HEAD 2>/dev/null || echo unknown)"
  printf -- '- **Driver PID:** %s EXITED\n' "$DRIVER_PID"
  printf -- '- **Rescore:** %s/22 — [%s](./.codex-r8-force16-rescore.log)\n' "$pass_count" ".codex-r8-force16-rescore.log"
  printf -- '- **Poll log:** [%s](./.codex-r8-force16-poll.log)\n' ".codex-r8-force16-poll.log"
} >"$CHECKPOINT"

{
  printf '\n### Poll checkpoint %s (force16 exit)\n\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  printf '| Field | Value |\n|-------|-------|\n'
  printf '| **Driver** | EXITED PID **%s** |\n' "$DRIVER_PID"
  printf '| **Rescore** | **%s/22** — [.codex-r8-force16-rescore.log](./.codex-r8-force16-rescore.log) |\n' "$pass_count"
