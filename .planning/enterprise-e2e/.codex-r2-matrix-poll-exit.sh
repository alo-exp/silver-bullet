#!/usr/bin/env bash
# Poll Round Codex-2 Tier C matrix driver until exit; full rescore + ledger update.
set -euo pipefail
...
INTERVAL="${2:-75}"
POLL_LOG="${ROOT}/.planning/enterprise-e2e/.codex-r2-matrix-poll.log"
RESCORE_LOG="${ROOT}/.planning/enterprise-e2e/.codex-r2-matrix-rescore.log"
MATRIX_LOG="${ROOT}/.e2e-matrix-codex-live.log"
LEDGER="${ROOT}/.planning/enterprise-e2e/ROUND-CODEX-2-LEDGER.md"
CHECKPOINT="${ROOT}/.planning/enterprise-e2e/.codex-r2-matrix-checkpoint.md"
...
{
  printf '# Round Codex-2 Tier C checkpoint\n\n'
  printf -- '- **When:** %s *(EXIT)*\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  printf -- '- **Branch:** enterprise-e2e/codex @ %s\n' "$(git rev-parse --short HEAD 2>/dev/null || echo unknown)"
  printf -- '- **Driver PID:** %s EXITED\n' "$DRIVER_PID"
  printf -- '- **Rescore:** %s/22 — [.codex-r2-matrix-rescore.log](./.codex-r2-matrix-rescore.log)\n' "$pass_count"
  printf -- '- **Poll log:** [.codex-r2-matrix-poll.log](./.codex-r2-matrix-poll.log)\n'
} >"$CHECKPOINT"

{
  printf '\n### Poll checkpoint %s (Round Codex-2 Tier C exit)\n\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  printf '| Field | Value |\n|-------|-------|\n'
  printf '| **Driver** | EXITED PID **%s** |\n' "$DRIVER_PID"
  printf '| **Matrix rescore** | **%s/22** — [.codex-r2-matrix-rescore.log](./.codex-r2-matrix-rescore.log) |\n' "$pass_count"
