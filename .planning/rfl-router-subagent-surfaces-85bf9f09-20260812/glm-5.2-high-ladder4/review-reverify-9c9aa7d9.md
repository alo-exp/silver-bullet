# RFL Ladder 4 — GLM 5.2 High re-verify @ `9c9aa7d9`

**Rung:** `sb-glm-5-2-high` (GLM 5.2 High). **REVIEWER ONLY.**
...
**Official High rung:** missing on this freeze (this rung is the High fill-in).
**Peer CLEAN on this SHA (parent-briefed):** Opus Extra High, Opus Max, GPT Max, Grok 4.6 Extra High.
...
Both copies byte-identical. Equal to the briefed frozen SHA at **start and end**. No HASH MISMATCH.

**Tooling:** `graphify query` ran first (plan / clarify / prior ladder-4 reviews — surfaced Opus XHigh `aa3677a5`, Opus XHigh `71427c3d` / `a8e7a463`, Grok 4.6 XHigh `9c9aa7d9` prior CLEAN on this SHA). agentmemory `memory_save` is **not registered** in this session. Native Read used for line cites.
...
|---|---|
| `nested_executor` lock-only (not a catalog JSON field; schema unchanged) | L118, L120, L122, L175, L750 |
| B1 / `docs/apo-catalog.schema.json` unchanged (`additionalProperties: false`) | L118, L120, L259 |
| Public `/sb` | L110, L120, L173 |
| Catalog generated (`generate-apo-catalog.py` / `build_catalog()`; not hand-edit SOT) | L118, L175, L750 |
| Unlimited **tree** nesting; recursive cycles fail-closed | L122, L263, L630, L727 |
| DFS **tri-color / recursion-stack** (visited-set alone insufficient) | L122, L263, L630, L727 |
| Two-limb in-plan Executor mint: (a) Work Plan–cited **or** (b) pre-existing catalog WF supporting that cited node | L112, L118, L251, L265 |
| Mid-I new PUB-01 definition / new catalog WF record → **row 40, not row 37** | L112, L118, L122, L251, L265, **L666**, **L669**, L737, L859 |
| Remint mints a **new `launch_id`** | L124, L243, L251, L265, L433, L669, L730, L737 |
| Exclusive `hooks/lib/wbs-projector.sh` packet writer; admission **requests**, is not a second writer | L173, L263, L429, L457, L542, L617, L738, L764 |
| FAST is **not a Job** (classify + catalog dispatch, not WF mint) | L116, L122, L237, L259, L548 |
| Authorizer is **not** Approver | L261 (and L187/L261 cited by peers) |
| ESC-02 **no A** | L124, L611, L731 |
| `prompt_hash` inner-prompt bytes only | L241, L433, L592, L263 |
...
| Limb (b) = **observable post-revoke effects** only | L251, L265, L630, L669, L737, L859 |
...
L173: launch adapters persist work-spec/plan artifacts **only by invoking** `hooks/lib/wbs-projector.sh`; that helper is the only writer of WBS, packet, work-spec, and plan-artifact files. `hooks/lib/orchestrator-admission.sh` **requests** that projector to persist `context-refs-snapshot/` and is **not** a second packet writer. Same lock at L263 (admission recompute/stamp), L429, L457, L542, L764.
...
L112: Executor `wf_mint` / `wf_invoke` is legal **iff** it **invokes/instantiates** (a) a Work Plan–cited WF/AF (`plan_node_id` / WBS id from the validated plan) **or** (b) a **pre-existing catalog** WF that supports that cited node. Creating a **new PUB-01 definition / new catalog WF record** mid-I is **out of plan** → `blocked_executor_wf_out_of_plan` (row 40). Uncited / new product scope is the same row 40. Twin at L118.
...
L263 (canonical): GC when **either** (1) `launch_id` is **CAS-provably superseded** **or** (2) durable **`scope_complete` / `completion_receipt_id`** is CAS-recorded. Do **not** wait for fence release or child terminality / process-death (L598 / pid-exists / OFF-01 post-MVP). Missing snapshot for a still-current incomplete id is row 4 / corrupt, not successful GC. Same two limbs at L433, L592, L728, L738 (`VAL/TST-RFL-626`).

### Special-file snapshot → exactly row 4 — **PASS**

L263 + L633: fifo/socket/device, dangling symlink, symlink loop → **row 4** `blocked_launch_prompt_spec` (cannot form a valid snapshot; **not** row 1). L630 explicitly excludes those from row 1 ("non-regular snapshot entries at admit ... — those are row 4 `blocked_launch_prompt_spec`"). L738 pins fifo/socket/device/dangling/loop FAIL → row 4 (not row 1).

### Lock emitter `scripts/generate-router-contract-locks.py` — **PASS**

L175: that script emits `contracts/public-workflow-routes.lock.json` and `contracts/apo-hierarchy.lock.json`; hand-authored `nested_executor` table stays hand-authored (this script does not invent that table). Not `generate-apo-artifacts.py` / not catalog builders.

### L511 in-plan — **PASS**

L511 mermaid edge: `Exec -->|in-plan wf_mint / wf_invoke| NwInsert["Authorizer-admitted in-plan nested WF (no return to /sb)"]`. Matches L112 / L156. Orchestrator/`/sb` still do not mint WFs.
