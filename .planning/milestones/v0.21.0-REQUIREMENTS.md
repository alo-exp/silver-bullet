...
- [ ] **HOOK-02**: `dev-cycle-check.sh` state-tamper detection must check that the write destination of a command points inside `${SB_RUNTIME_HOME_ROOT}/.silver-bullet/`, not whether the path string appears anywhere in the command text (including heredoc body content). Resolves GitHub #8.
