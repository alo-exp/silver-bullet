# Brief — Rung 06 review pass 12 (Pi Codex GPT-5.6 Sol Extra High)

**Rung:** 6 of 8 — **twelfth review pass** on this Extra High model (Policy F: Extra High streak is **0** after launcher `--record-rung-review-outcome accept-apply` for pass 11 `R6k-F01` MED; this is consecutive CLEAN attempt **1** on the **post-R6k** freeze). High’s streak of 2 is a **different** `rung-id` (`rung-05-pi-codex-gpt-5.6-sol-high`) and is complete — do **not** re-review High.
...
- Fresh Extra High invoke. Do **not** `--continue`. Pass 11 first hop died at 600s without idle-timeout env; this hop is a new session with `PI_RUN_TIMEOUT=7200` and matching idle timeouts.

Pass 1 history is **`review.md`** (CLEAN on pre-R6b SHA `d45ccf6b…`, no `R6-F*`). Pass 2 history is **`review-rerun-2.md`** (NOT CLEAN; `R6b-F01` HIGH). Pass 3 history is **`review-rerun-3.md`** (NOT CLEAN; `R6c-F01` HIGH). Pass 4 history is **`review-rerun-4.md`** (NOT CLEAN; `R6d-F01` HIGH). Pass 5 history is **`review-rerun-5.md`** (CLEAN on post-R6d SHA `1f11eacc…`; no `R6e-F*`). Pass 6 history is **`review-rerun-6.md`** (NOT CLEAN; `R6f-F01` MED; ACCEPT-apply). Pass 7 history is **`review-rerun-7.md`** (CLEAN on post-R6f SHA `f7c632b8…`; no `R6g-F*`; `verify_1-rerun-7.md` PASS; `verify_2-rerun-7.md` PASS; launcher recorded streak **1**). Pass 8 history is **`review-rerun-8.md`** (NOT CLEAN; `R6h-F01` MED; ACCEPT-apply; `verify_1-rerun-8.md` PASS; `verify_2-rerun-8.md` PASS; Policy C ACCEPT-apply; streak reset to **0**). Pass 9 history is **`review-rerun-9.md`** (NOT CLEAN; `R6i-F01` MED + `R6i-F02` MED; ACCEPT-apply; `verify_1-rerun-9.md` PASS; `verify_2-rerun-9.md` PASS; Policy C ACCEPT-apply; streak reset to **0**). Pass 10 history is **`review-rerun-10.md`** (NOT CLEAN; `R6j-F01` MED + `R6j-F02` MED; ACCEPT-apply; `verify_1-rerun-10.md` PASS; `verify_2-rerun-10.md` PASS; Policy C ACCEPT-apply; streak reset to **0**). Pass 11 history is **`review-rerun-11.md`** (NOT CLEAN; `R6k-F01` MED; ACCEPT-apply; `verify_1-rerun-11.md` PASS; `verify_1-apply-rerun-11.md` PASS; Policy C ACCEPT-apply; streak reset to **0**). Do **not** overwrite `review.md`, `review-rerun-2.md`, `review-rerun-3.md`, `review-rerun-4.md`, `review-rerun-5.md`, `review-rerun-6.md`, `review-rerun-7.md`, `review-rerun-8.md`, `review-rerun-9.md`, `review-rerun-10.md`, or `review-rerun-11.md`. Write **`review-rerun-12.md`** only.

## Why this pass exists

Extra High pass 11 (`review-rerun-11.md`) was **NOT CLEAN** (`R6k-F01` MED) on pin **`1b681ea7…`**. `verify_1-rerun-11.md` PASS independently sustained that hole. Parent APPLYed R6k-F01 per [`APPLY-rerun-11.md`](APPLY-rerun-11.md). `verify_1-apply-rerun-11.md` PASS. New freeze pin is **`bdb5c916…`**. Policy F Extra High streak is **0** (required **2** consecutive CLEAN on this Extra High model on the **current** freeze before Claude). This is **pass 12** — first CLEAN attempt on the **post-R6k pin**. **New SHA; residual only.** Confirm R6k landed as actually encoded, then hunt residuals. Do **not** rubber-stamp pass 11. Do **not** re-file `R6k-F01` unless a residual defect remains in **this** freeze text.

**Independent residual re-hunt is mandatory.** Do **not** copy `review.md` or `review-rerun-2.md`–`review-rerun-11.md`. Re-read the pinned freeze from scratch. Passes 1–11 are history, not authority. Residual only: do not re-file APPLYed IDs unless a residual defect remains in **this** freeze text. If you find a residual template-contract gap, file it as `R6l-F01+`. If you find none, say **CLEAN** with evidence from **this** pass’s freeze read.

High APPLY history already in this freeze (do not unwind): R5-F01–F03, R5b-F01–F03, R5c-F01–F03, R5e-F01, R5f-F01, R5h-F01, R5i-F01, R5j-F01, R5k-F01. Earlier rungs: R1-F01–F10, R2-F01–F06, R3-F01–F05, R1b-F01–F03. Extra High pass 2 APPLY: **R6b-F01**. Extra High pass 3 APPLY: **R6c-F01**. Extra High pass 4 APPLY: **R6d-F01**. Extra High pass 6 APPLY: **R6f-F01**. Extra High pass 8 APPLY: **R6h-F01**. Extra High pass 9 APPLY: **R6i-F01**, **R6i-F02**. Extra High pass 10 APPLY: **R6j-F01**, **R6j-F02**. Extra High pass 11 APPLY: **R6k-F01**. Extra High pass 1, pass 5, and pass 7 filed **no** new IDs. Prior IDs **R6b–R6k are settled** (R6k-F01 just applied).
...
## Independent re-hunt (do not skip because R6k just APPLYed)

Re-scan the pinned **post-R6k** freeze for **new** residual gaps that Extra High pass 11 could have missed, **and** for any **residual** hole in APPLYed contracts. Hunt these classes even if you expect none; file only if evidence is in **this** freeze text:

1. **Coverage Matrix REQ-list / edge-set residual (R6k-F01 APPLY) — primary hunt.** Confirm named **`coverage-matrix-req-cell-list`** (`, ` = U+002C + U+0020; atoms exact `REQ-[0-9]{2}`). Confirm matrix `AC` cell is **exactly one** exact `AC-[0-9]{2}`. Confirm normative **matrix ↔ Functional edge-set equality** (fail closed). Confirm bind to QC-8 (`REQ-F70` mismatch FAIL, not advisory), Wave 3 Step 8 serialize+parse, `review-cross-artifact`, compiler/migration tests. Confirm fixture PASS: Functional `REQ-01`/`AC-01` plus `REQ-02`/`AC-01` with matrix `AC-01 | REQ-01, REQ-02`. Confirm fixture FAIL: `REQ-01,REQ-02`; semicolon/slash/pipe/whitespace aliases; exact-ID-but-wrong-pair. Confirm malformed staged matrix cannot install. Hunt leaks: grammar named in locked-contract / ID scheme but omitted from QC-8 or Step 8 serialize or XART; QC-8 still advisory on mismatch; matrix AC cell still accepting lists; edge-set equality missing one direction; `REQ-01,REQ-02` still PASS; exact-ID-but-wrong-pair still installing; R6j Functional AC emit / `nfr-source-cell-list` weakened to match a looser matrix parser. File a **new** `R6l-F*` only if a defect remains **after** the APPLY text above is present. Do **not** re-file R6k-F01.
2. **Step 8 / XART AC-cell cardinality residual (R6j-F01 APPLY).** Confirm R6i one-`AC-nn`-per-cell is bound to Wave 3 **Step 8**, **`review-cross-artifact`**, and compiler/migration tests. Confirm emitted Functional cell is **exactly one** exact `AC-[0-9]{2}`. Confirm list aliases (comma, semicolon, slash, pipe, whitespace-separated lists) are prohibited. Confirm `AC-01` PASS and `AC-01, AC-02` FAIL at **mint/serialize/XART** (not Wave 1 template tests only). Confirm malformed cell cannot install. Confirm Wave 3 / Wave 6 inherited pins include `R6h-F01` / `R6i-F01` / `R6j-F01`. File a **new** `R6l-F*` only if a defect remains **after** the APPLY text above is present. Do **not** re-file R6j-F01.
3. **Step 8 / XART `nfr-source-cell-list` residual (R6j-F02 APPLY).** Confirm named **`nfr-source-cell-list`** (`, ` = U+002C + U+0020) is bound to Step 8 serialize+parse **and** `review-cross-artifact` reverse-coverage / exclusivity / overlap (same parser as Wave 1). Confirm `QA-01, SLO-01` parses as two IDs; `QA-01,SLO-01` (no space) FAIL. Confirm overlap second-atom is detectable only through correct list parsing. Confirm malformed staged Source cannot install. Confirm Wave 3 / Wave 6 inherited pins include `R6i-F02` / `R6j-F02`. File a **new** `R6l-F*` only if a defect remains **after** the APPLY text above is present. Do **not** re-file R6j-F02.
4. **Functional AC-cell cardinality residual (R6i-F01 APPLY).** Confirm one Functional `AC` cell = **exactly one** `AC-nn`. Confirm `AC-01, AC-02` FAIL. Confirm many-to-one REQ↔AC is via **multiple Functional rows**, not a comma list in one cell. Confirm Wave 1 parse + Wave 2 QC-4/`REQ-F30` consume that cardinality. File a **new** `R6l-F*` only if a defect remains **after** the APPLY text above is present. Do **not** re-file R6i-F01.
5. **NFR Source cell-list grammar residual (R6i-F02 APPLY).** Confirm named **`nfr-source-cell-list`**: atoms separated by `, ` (U+002C COMMA + exactly one U+0020 SPACE); no other whitespace. Each atom exact `QA-[0-9]{2}` / `SLO-[0-9]{2}` / `CTRL-[0-9]{2}` / `SCAN:<section>#<line-or-id>` (`<section>` and `<line-or-id>` non-empty, no comma, no space). Same parser for reverse-coverage / exclusivity / overlap FAIL. Live Source example required (not header-only empty). Fixture PASS: `QA-01, SLO-01` parses as two IDs. Fixture FAIL: `QA-01,SLO-01` (missing space). File a **new** `R6l-F*` only if a defect remains **after** the APPLY text above is present. Do **not** re-file R6i-F02.
6. **Functional AC cells residual (R6h-F01 APPLY).** Confirm Wave 1 Functional REQUIREMENTS `AC` column **cells** are exact `AC-nn` (e.g. `AC-01`), not header-only `AC`; template/min fixture includes an `AC-01` cell; live `Acceptance Criterion` column (or equivalent old heading) is forbidden on Functional rows; `test-spec-req-id-parse.sh` parses the Functional AC cell (`AC-[0-9]{2}`), not SPEC `**AC-01**` alone. Confirm Wave 2 QC-4 has a **behavioral** fixture (not skill-string only) that `REQ-F30` does **not** fire on a valid `AC-nn` join key; a valid `AC-01` cell PASSes that check; live `Acceptance Criterion` column FAIL. File a **new** `R6l-F*` only if a defect remains **after** the APPLY text above is present. Do **not** re-file R6h-F01.
7. **ID-namespace exhaustion residual (R6f-F01 APPLY).** Confirm the named **ID-namespace exhaustion fail-closed**: allocatable domain `00–99` inclusive (`-00` allocatable) for every exact two-digit prefix already required (`AC-nn`, `EX-nn`, every catalog prefix, `REQ-nn`, `NFR-nn`); when next-free cannot mint an unused exact two-digit ID (all `00–99` live **or** tombstoned for that prefix), **FAIL closed** before any canonical pair replace; do not wrap, do not three-digit, do not reuse tombstones. Confirm Step **7** and Step **8** both carry the rule. File a **new** `R6l-F*` only if a defect remains **after** the APPLY text above is present. Do **not** re-file R6f-F01.
8. **Fixed-point residual (R6d-F01 APPLY).** Confirm the named **fixed-point**: after a successful 7a/8a mutation, Step 8 / 7a/8a / `review-cross-artifact` must re-PASS on the **exact staged bytes** before install; mutate-after-PASS is stale until revalidated. Hunt leaks: 8a (or 7a) still allowed to mutate after a pair PASS with install proceeding on stale PASS evidence; “re-run reviews” that omit `review-cross-artifact` or Step 8 allocator/tombstone/lineage/coverage/Source/disposition/version/**exhaustion**/**Functional AC cell**/**nfr-source-cell-list**/**coverage-matrix-req-cell-list** checks. File a **new** `R6l-F*` only if a defect remains **after** the APPLY text above is present. Do **not** re-file R6d-F01.
9. **Recoverable pair-install residual (R6c-F01 APPLY).** Confirm 7a/8a and intervening QC still consume **staged** candidates (`source_inputs` = staged SPEC), not only canonical paths; 7a/8a FAIL MUST NOT install; snapshot both canonicals (including absence) before mutating either; second-replace failure restores prior bytes of both. File a **new** `R6l-F*` only if a defect remains **after** R6c, R6d, R6f, R6h, R6i, R6j, **and** R6k APPLY text is present. Do **not** re-file R6c-F01.
10. **Staged pair commit residual (R6b-F01 APPLY).** Confirm Step 7 still MUST NOT durable-commit canonical SPEC (staging only) until Step 8 succeeds; both files replace together; Step 8 FAIL (including **exhaustion**, **invalid Functional AC cell**, **malformed `nfr-source-cell-list`**, **malformed `coverage-matrix-req-cell-list`**, and **matrix↔Functional edge-set mismatch**) leaves prior SPEC unchanged (greenfield: both unwritten). File a **new** `R6l-F*` only if a defect remains **after** R6b, R6c, R6d, R6f, R6h, R6i, R6j, **and** R6k APPLY text is present. Do **not** re-file R6b-F01.
11. NFR exclusive-overlap residual (R5k-F01 APPLY): exclusive branches still stated as **either** live NFR Source **or** exactly one Source Dispositions row — **not both**; named overlap FAIL on review-requirements, review-cross-artifact, and Step 8 (now fail-before **canonical pair replace** if unresolved); QA-01 live Source **and** `out-of-scope`/`deferred` negative fixture still present; inclusive-or leak that would still accept overlap. File a **new** `R6l-F*` only if a defect remains **after** exclusivity, staged pair commit, recoverable pair-install, fixed-point, exhaustion fail-closed, Functional AC cells, **one-per-cell**, **`nfr-source-cell-list`**, **R6j Step 8/XART binding**, **and R6k matrix REQ-list / edge-set** are present.
12. Greenfield / partial-pair residual (R5j-F01): true greenfield still predicates on **both** files absent (not SPEC-only); Wave 6 step **1b** SPEC-absent / REQUIREMENTS-present is named **preserve-or-fail-closed** (union prior REQUIREMENTS `id-tombstones` **or** fail before write — no silent wipe, no `[]` init, no partial output); Step 8 unions prior tombstones on **every** replace including 1b **and** 2/3/4b; fixture no SPEC + `id-tombstones: [REQ-03, NFR-02]` must not become `[]` / must not later mint `REQ-03`. Do not re-file R5j unless **this** freeze text still has a hole.
13. REQUIREMENTS tombstone residual (R5i-F01): REQUIREMENTS YAML `id-tombstones` shape vs live `REQ-nn`/`NFR-nn`; QC-2/QC-3 reissue FAIL vs Step 8 next-free skipping holes; Wave 6 augment persistence/honor vs true-greenfield `[]`; never-drop vs silently shrinking list; canonical allocator in REQUIREMENTS vs SPEC catalog tombstones leaking REQ/NFR or vice versa. Exhaustion must **not** free slots by dropping tombstones. Do not re-file R5i unless **this** freeze text still has a hole.
14. SPEC tombstone residual (R5h-F01): catalog/core prefixes still covered; QC-13/QC-12 reissue FAIL vs Step 7 next-free; Wave 6 branches 2/3/4b persist/honor; prefixes beyond `AC`/`EX` (US/OQ/OOS/CTRL/SLO/etc.). Exhaustion must **not** free slots by dropping tombstones. Do not re-file R5h unless **this** freeze text still has a hole.
15. Template contract: required vs optional headings; ID uniqueness/shape beyond QC-13 **and** QC-2 two-digit REQ/NFR **and** the `EX-nn` grammar (parser holes, Coverage Matrix / ROADMAP vs compiler mint, `P1–P3` vs ID width, other required packs still missing prefixes); GWT; invariants; Change History beyond QC-10; decision log; NFR reverse+forward **after exclusivity** and **`nfr-source-cell-list`** **and R6j Step 8/XART parse**; Coverage Matrix **after `coverage-matrix-req-cell-list` + edge-set equality**; Source Dispositions vs `SCAN:` sources; security/telemetry/API/UX/data/errors packs; whether other finite exact-width namespaces lack exhaustion fail-closed after R6f; whether Functional AC cells still leak into NFR Metric / Coverage Matrix Notes; whether AC-cell lists vs NFR Source lists vs matrix REQ lists still share a parser by accident.
16. Kind catalog + Clarify skip-turns: required-pack bodies (QC-12) including `EX-nn` vs **optional**-present Examples; `multi` union completeness; QC-6b vs catalog membership; `_TBD` vs QC-1; Step 7 mint/preserve vs unlabeled/malformed/duplicate `EX-*`. How Clarify `--spec` asks only relevant turns per kind.
17. Compiler/QC/tests/v0.35 lock: Wave 3 Step 7 vs Wave 6 branches 1 / **1b** / 2 / 3 / 4b fail-before-write holes **after** staged pair commit, recoverable pair-install, fixed-point, exhaustion fail-closed, Functional AC cells, **one-per-cell**, **`nfr-source-cell-list`**, **R6j Step 8/XART binding**, **and R6k matrix REQ-list / edge-set**; named fault codes vs string-only asserts; Coverage Matrix vs unlabeled/dup AC; QC-2 exact-width vs any remaining “one or more digits” baseline leak; QC-12/QC-13 prefix lists vs pack table (any other required pack still without an ID); Step 8 vs review-requirements vs Coverage Matrix vs ROADMAP on REQ/NFR tombstones; overlap FAIL fixture still named in Wave 2 QC strings; 7a/8a FAIL + commit-boundary + mutate-after-PASS + **full-namespace mint FAIL** + **REQ-F30 no-fire on AC-01** + **`AC-01, AC-02` FAIL at mint/serialize/XART** + **`QA-01, SLO-01` PASS / `QA-01,SLO-01` FAIL on Step 8 and XART** + **matrix `REQ-01, REQ-02` PASS / `REQ-01,REQ-02` FAIL / exact-ID-but-wrong-pair FAIL** fixtures still named.
18. Plan-hygiene last — only if it breaks the template contract.
...
1. `graphify query "agent-pi invoke gpt-5.6-sol-xhigh coverage-matrix-req-cell-list QC-8 matrix Functional edge-set R6k"` (or a scoped template/kind-pack query) before exploring. Retrieve prior notes via Graphify, not raw agentmemory dumps.
...
For each: ID (`R6l-F01+`), severity HIGH|MED|LOW|NIT, location (heading/wave/pack/kind), evidence quote, why it matters for the **template contract** (or plan, if secondary), suggested freeze-text fix. Then: **CLEAN** or **NOT CLEAN**.
...
- Official review: `.planning/rfl-spec-template-world-class/rung-06-pi-codex-gpt-5.6-sol-xhigh/review-rerun-12.md` only.
- Do **not** overwrite `review.md`. Do **not** overwrite `review-rerun-2.md`. Do **not** overwrite `review-rerun-3.md`. Do **not** overwrite `review-rerun-4.md`. Do **not** overwrite `review-rerun-5.md`. Do **not** overwrite `review-rerun-6.md`. Do **not** overwrite `review-rerun-7.md`. Do **not** overwrite `review-rerun-8.md`. Do **not** overwrite `review-rerun-9.md`. Do **not** overwrite `review-rerun-10.md`. Do **not** overwrite `review-rerun-11.md`. Do **not** overwrite High `rung-05-*` files.
- Do **not** write live `templates/` or `skills/` patches. Freeze-text suggestions belong in `review-rerun-12.md`.
