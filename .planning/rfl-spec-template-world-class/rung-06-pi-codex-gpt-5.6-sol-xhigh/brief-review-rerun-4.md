# Brief — Rung 06 review pass 4 (Pi Codex GPT-5.6 Sol Extra High)

**Rung:** 6 of 8 — **fourth review pass** on this Extra High model (Policy F: Extra High streak reset to **0** after pass 3 ACCEPT-apply of R6c-F01; this is consecutive CLEAN attempt **1** on the **post-R6c** freeze). High’s streak of 2 is a **different** `rung-id` (`rung-05-pi-codex-gpt-5.6-sol-high`) and is complete — do **not** re-review High.
...
Pass 1 history is **`review.md`** (CLEAN on pre-R6b SHA `d45ccf6b…`, no `R6-F*`). Pass 2 history is **`review-rerun-2.md`** (NOT CLEAN; `R6b-F01` HIGH). Pass 3 history is **`review-rerun-3.md`** (NOT CLEAN; `R6c-F01` HIGH). Do **not** overwrite `review.md`, `review-rerun-2.md`, or `review-rerun-3.md`. Write **`review-rerun-4.md`** only.

## Why this pass exists

Extra High pass 3 (`review-rerun-3.md`) was **NOT CLEAN** (`R6c-F01` HIGH). `verify_1-rerun-3.md` PASS and `verify_2-rerun-3.md` PASS independently sustained that finding on the **pre-APPLY** freeze (`87830186…`). Launcher ACCEPT-applied R6c-F01 (see `APPLY-rerun-3.md`) and recorded `--record-rung-review-outcome accept-apply` → Extra High streak **0**. Policy F requires **two consecutive** CLEAN reviews on **this same Extra High model** on the **current** freeze before Claude. This is **pass 4** — first CLEAN attempt on the **post-R6c** pin. REJECT does not break a zero streak; any new ACCEPT resets it after APPLY.

**Independent residual re-hunt is mandatory.** Do **not** rubber-stamp pass 1 CLEAN (different SHA). Do **not** copy `review.md`, `review-rerun-2.md`, or `review-rerun-3.md`. Re-read the pinned freeze from scratch. Passes 1–3 are history, not authority. Residual only: do not re-file APPLYed IDs unless a residual defect remains in **this** freeze text. If you find a residual template-contract gap, file it as `R6d-F01+`. If you find none, say **CLEAN** with evidence from **this** pass’s freeze read.

High APPLY history already in this freeze (do not unwind): R5-F01–F03, R5b-F01–F03, R5c-F01–F03, R5e-F01, R5f-F01, R5h-F01, R5i-F01, R5j-F01, R5k-F01. Earlier rungs: R1-F01–F10, R2-F01–F06, R3-F01–F05, R1b-F01–F03. Extra High pass 2 APPLY: **R6b-F01**. Extra High pass 3 APPLY: **R6c-F01**. Extra High pass 1 filed **no** new IDs.
...
This SHA is **post R6c APPLY** (pre-APPLY pin was `878301866ecbc51c2ee144d566fdfaa36fcb34b37d8fd88166723b32d92879f5`; pre-R6b pin was `d45ccf6b686250638ba22778618c4f31761919c5fd00ef692569049ac6526401`). Twins are byte-identical at this pin.
...
R1-F01–F10, R2-F01–F06, R3-F01–F05, R1b-F01–F03, **R5-F01–F03**, **R5b-F01–F03**, **R5c-F01–F03**, **R5e-F01**, **R5f-F01**, **R5h-F01**, **R5i-F01**, **R5j-F01**, **R5k-F01**, **R6b-F01**, and **R6c-F01** are already in this freeze. Do **not** re-file those IDs unless a **residual defect remains in this freeze text**.

New finding IDs: **R6d-F01+** (Extra High pass 4). Do **not** reuse `R6c-F*` / `R6b-F*` / `R6-F*` / `R5*` / `R5m-F*` / `R5l-F*` / High rerun IDs.
...
1. `graphify query "agent-pi invoke gpt-5.6-sol-xhigh recoverable pair-install snapshot-restore 7a 8a staged R6c-F01"` (or a scoped template/kind-pack query) before exploring. Retrieve prior notes via Graphify, not raw agentmemory dumps.
...
For each: ID (`R6d-F01+`), severity HIGH|MED|LOW|NIT, location (heading/wave/pack/kind), evidence quote, why it matters for the **template contract** (or plan, if secondary), suggested freeze-text fix. Then: **CLEAN** or **NOT CLEAN**.
...
- Official review: `.planning/rfl-spec-template-world-class/rung-06-pi-codex-gpt-5.6-sol-xhigh/review-rerun-4.md` only.
- Do **not** overwrite `review.md`. Do **not** overwrite `review-rerun-2.md`. Do **not** overwrite `review-rerun-3.md`. Do **not** overwrite High `rung-05-*` files.
