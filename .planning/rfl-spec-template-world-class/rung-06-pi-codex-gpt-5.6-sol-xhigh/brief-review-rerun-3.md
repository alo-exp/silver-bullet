# Brief — Rung 06 review pass 3 (Pi Codex GPT-5.6 Sol Extra High)

**Rung:** 6 of 8 — **third review pass** on this Extra High model (Policy F: Extra High streak reset to **0** after pass 2 ACCEPT-apply of R6b-F01; this is consecutive CLEAN attempt **1** on the **post-R6b** freeze). High’s streak of 2 is a **different** `rung-id` (`rung-05-pi-codex-gpt-5.6-sol-high`) and is complete — do **not** re-review High.
...
Pass 1 history is **`review.md`** (CLEAN on pre-R6b SHA `d45ccf6b…`, no `R6-F*`). Pass 2 history is **`review-rerun-2.md`** (NOT CLEAN; `R6b-F01` HIGH). Do **not** overwrite `review.md` or `review-rerun-2.md`. Write **`review-rerun-3.md`** only.

## Why this pass exists

Extra High pass 2 (`review-rerun-2.md`) was **NOT CLEAN** (`R6b-F01` HIGH). `verify_1-rerun-2.md` PASS and `verify_2-rerun-2.md` PASS independently sustained that finding on the **pre-APPLY** freeze. Launcher ACCEPT-applied R6b-F01 and recorded `--record-rung-review-outcome accept-apply` → Extra High streak **0**. Policy F requires **two consecutive** CLEAN reviews on **this same Extra High model** on the **current** freeze before Claude. This is **pass 3** — first CLEAN attempt on the **post-R6b** pin. REJECT does not break a zero streak; any new ACCEPT resets it after APPLY.

**Independent residual re-hunt is mandatory.** Do **not** rubber-stamp pass 1 CLEAN (different SHA). Do **not** copy `review.md` or `review-rerun-2.md`. Re-read the pinned freeze from scratch. Pass 1 and pass 2 are history, not authority. Residual only: do not re-file APPLYed IDs unless a residual defect remains in **this** freeze text. If you find a residual template-contract gap, file it as `R6c-F01+`. If you find none, say **CLEAN** with evidence from **this** pass’s freeze read.

High APPLY history already in this freeze (do not unwind): R5-F01–F03, R5b-F01–F03, R5c-F01–F03, R5e-F01, R5f-F01, R5h-F01, R5i-F01, R5j-F01, R5k-F01. Earlier rungs: R1-F01–F10, R2-F01–F06, R3-F01–F05, R1b-F01–F03. Extra High pass 2 APPLY: **R6b-F01**. Extra High pass 1 filed **no** new IDs.
...
This SHA is **post R6b APPLY** (pre-APPLY pin was `d45ccf6b686250638ba22778618c4f31761919c5fd00ef692569049ac6526401`). Twins are byte-identical at this pin.
...
R1-F01–F10, R2-F01–F06, R3-F01–F05, R1b-F01–F03, **R5-F01–F03**, **R5b-F01–F03**, **R5c-F01–F03**, **R5e-F01**, **R5f-F01**, **R5h-F01**, **R5i-F01**, **R5j-F01**, **R5k-F01**, and **R6b-F01** are already in this freeze. Do **not** re-file those IDs unless a **residual defect remains in this freeze text**.

New finding IDs: **R6c-F01+** (Extra High pass 3). Do **not** reuse `R6b-F*` / `R6-F*` / `R5*` / `R5m-F*` / `R5l-F*` / High rerun IDs.
...
## Independent re-hunt (do not skip because R6b APPLY landed)

Re-scan the pinned freeze for **new** residual gaps that Extra High pass 2 could have missed **after** R6b-F01 APPLY, **and** for any **residual** hole in APPLYed contracts. Hunt these classes even if you expect none; file only if evidence is in **this** freeze text:

1. **Staged pair commit residual (R6b-F01 APPLY) — primary hunt.** Confirm Step 7 still MUST NOT durable-commit canonical SPEC (staging only) until Step 8 succeeds; both files replace together; Step 8 FAIL leaves prior SPEC unchanged (greenfield: both unwritten). Hunt leaks: Wave 6 step 1 still saying “write as today” in a way that durable-commits SPEC first; Step 7a/8a 2-pass writing canonical files mid-path; inverse pair (REQUIREMENTS absent / SPEC present) silent wipe or lone-file commit; pair replace that can leave lone REQUIREMENTS; fixture missing on greenfield **or** augment 2/3/4b; 1b FAIL no longer preserve-or-fail-closed; “fail before replacing REQUIREMENTS” still the only guard on some path; filesystem non-atomic replace without rollback. File a **new** `R6c-F*` only if a defect remains **after** the APPLY text above is present. Do **not** re-file R6b-F01.
2. NFR exclusive-overlap residual (R5k-F01 APPLY): exclusive branches still stated as **either** live NFR Source **or** exactly one Source Dispositions row — **not both**; named overlap FAIL on review-requirements, review-cross-artifact, and Step 8 (now fail-before **canonical pair replace** if unresolved); QA-01 live Source **and** `out-of-scope`/`deferred` negative fixture still present; inclusive-or leak that would still accept overlap. File a **new** `R6c-F*` only if a defect remains **after** exclusivity **and** staged pair commit are present.
...
5. SPEC tombstone residual (R5h-F01): catalog/core prefixes still covered; QC-13/QC-12 reissue FAIL vs Step 7 next-free; Wave 6 branches 2/3/4b persist/honor; prefixes beyond `AC`/`EX` (US/OQ/OOS/CTRL/SLO/etc.). Do not re-file R5h unless **this** freeze text still has a hole.
6. Template contract: required vs optional headings; ID uniqueness/shape beyond QC-13 **and** QC-2 two-digit REQ/NFR **and** the `EX-nn` grammar (parser holes, Coverage Matrix / ROADMAP vs compiler mint, `P1–P3` vs ID width, other required packs still missing prefixes); GWT; invariants; Change History beyond QC-10; decision log; NFR reverse+forward **after exclusivity**; Source Dispositions vs `SCAN:` sources; security/telemetry/API/UX/data/errors packs.
7. Kind catalog + Clarify skip-turns: required-pack bodies (QC-12) including `EX-nn` vs **optional**-present Examples; `multi` union completeness; QC-6b vs catalog membership; `_TBD` vs QC-1; Step 7 mint/preserve vs unlabeled/malformed/duplicate `EX-*`. How Clarify `--spec` asks only relevant turns per kind.
8. Compiler/QC/tests/v0.35 lock: Wave 3 Step 7 vs Wave 6 branches 1 / **1b** / 2 / 3 / 4b fail-before-write holes **after** staged pair commit; named fault codes vs string-only asserts; Coverage Matrix vs unlabeled/dup AC; QC-2 exact-width vs any remaining “one or more digits” baseline leak; QC-12/QC-13 prefix lists vs pack table (any other required pack still without an ID); Step 8 vs review-requirements vs Coverage Matrix vs ROADMAP on REQ/NFR tombstones; overlap FAIL fixture still named in Wave 2 QC strings.
9. Plan-hygiene last — only if it breaks the template contract.
...
1. `graphify query "agent-pi invoke gpt-5.6-sol-xhigh staged pair commit Step 7 Step 8 R6b-F01"` (or a scoped template/kind-pack query) before exploring. Retrieve prior notes via Graphify, not raw agentmemory dumps.
...
For each: ID (`R6c-F01+`), severity HIGH|MED|LOW|NIT, location (heading/wave/pack/kind), evidence quote, why it matters for the **template contract** (or plan, if secondary), suggested freeze-text fix. Then: **CLEAN** or **NOT CLEAN**.
...
- Official review: `.planning/rfl-spec-template-world-class/rung-06-pi-codex-gpt-5.6-sol-xhigh/review-rerun-3.md` only.
- Do **not** overwrite `review.md`. Do **not** overwrite `review-rerun-2.md`. Do **not** overwrite High `rung-05-*` files.
- Do **not** write live `templates/` or `skills/` patches. Freeze-text suggestions belong in `review-rerun-3.md`.
