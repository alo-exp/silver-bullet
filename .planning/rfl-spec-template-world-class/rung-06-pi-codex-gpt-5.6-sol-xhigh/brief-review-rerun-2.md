# Brief — Rung 06 review pass 2 (Pi Codex GPT-5.6 Sol Extra High)

**Rung:** 6 of 8 — **second review pass** on this Extra High model (Policy F: Extra High streak is **1** after pass 1 CLEAN was recorded; this is consecutive CLEAN attempt **2**). High’s streak of 2 is a **different** `rung-id` (`rung-05-pi-codex-gpt-5.6-sol-high`) and is complete — do **not** re-review High.
...
Pass 1 history is **`review.md`** (CLEAN, no `R6-F*`). Do **not** overwrite `review.md`. Write **`review-rerun-2.md`** only.

## Why this pass exists

Extra High pass 1 (`review.md`) was **CLEAN** (zero `R6-F*`). `verify_1.md` PASS and `verify_2.md` PASS independently confirmed that CLEAN on the **same** freeze SHA. Launcher recorded `--record-rung-review-outcome clean` → `consecutive_clean_reviews: 1` (required 2) for `rung-id` `rung-06-pi-codex-gpt-5.6-sol-xhigh`. Policy F still requires **two consecutive** CLEAN reviews on **this same Extra High model** before Claude. This is **pass 2** — consecutive CLEAN attempt **2** on the same post-R5k freeze (SHA unchanged; no APPLY after pass 1). REJECT does not break the streak; any new ACCEPT resets it after APPLY.

**Independent residual re-hunt is mandatory.** Do **not** rubber-stamp pass 1. Do **not** copy `review.md`. Re-read the pinned freeze from scratch. Pass 1 is history, not authority. Residual only: do not re-file APPLYed IDs unless a residual defect remains in **this** freeze text. If you find a residual template-contract gap, file it as `R6b-F01+`. If you find none, say **CLEAN** with evidence from **this** pass’s freeze read.

High APPLY history already in this freeze (do not unwind): R5-F01–F03, R5b-F01–F03, R5c-F01–F03, R5e-F01, R5f-F01, R5h-F01, R5i-F01, R5j-F01, R5k-F01. Earlier rungs: R1-F01–F10, R2-F01–F06, R3-F01–F05, R1b-F01–F03. Extra High pass 1 filed **no** new IDs.
...
This SHA is **unchanged since R5k APPLY** (same pin as Extra High pass 1, High pass 12, and High pass 13). Twins are byte-identical at this pin. No APPLY landed after Extra High pass 1.
...
New finding IDs: **R6b-F01+** (Extra High pass 2). Do **not** reuse `R6-F*` / `R5*` / `R5m-F*` / `R5l-F*` / High rerun IDs.
...
Findings that improve the template contract are in scope even when the wave text is tidy. Do **not** invent findings to avoid a second CLEAN. Do **not** invent findings to force Claude either.

## Tools (mandatory)

1. `graphify query "agent-pi invoke gpt-5.6-sol-xhigh NFR Source Dispositions exclusive overlap FAIL R5k-F01 residual R6b"` (or a scoped template/kind-pack query) before exploring. Retrieve prior notes via Graphify, not raw agentmemory dumps.
...
For each: ID (`R6b-F01+`), severity HIGH|MED|LOW|NIT, location (heading/wave/pack/kind), evidence quote, why it matters for the **template contract** (or plan, if secondary), suggested freeze-text fix. Then: **CLEAN** or **NOT CLEAN**.
...
- Official review: `.planning/rfl-spec-template-world-class/rung-06-pi-codex-gpt-5.6-sol-xhigh/review-rerun-2.md` only.
- Do **not** overwrite `review.md`. Do **not** overwrite High `rung-05-*` files.
