#!/usr/bin/env bash
# Pass 9 Claude High Pi launch. No set -u. No tee. No PIPESTATUS.
# Preserve invoke.sh exit code into review-attempt9-exit.txt.
...
LOG="$RUNG/logs/review-attempt9-stdout.txt"
EXITF="$RUNG/logs/review-attempt9-exit.txt"
REPO="/Users/shafqat/projects/silver-bullet/repo"

mkdir -p "$RUNG/logs"
rm -f "$RUNG/review-rerun-9.md" "$RUNG/review-rerun-9.md.IN_PROGRESS-stub-aside"

cd "$REPO"
bash "$REPO/scripts/agent-pi/invoke.sh" \
  --work-dir "$RUNG" \
  --brief-file "$RUNG/brief-review-rerun-9.md" \
  --expect-file "$RUNG/review-rerun-9.md" \
