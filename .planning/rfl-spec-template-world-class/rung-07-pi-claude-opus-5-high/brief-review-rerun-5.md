# Brief — Rung 07 review pass 5 (Pi Claude Opus 5 High)

**Rung:** 7 of 8 — **fifth review pass** on Claude Opus 5 High (Policy F: this rung's streak is **0** after pass 4 pack APPLY + launcher `--record-rung-review-outcome accept-apply`). Do **not** remap this Claude High hop onto Grok, Extra High, Cursor, Fast, or GPT. Do **not** launch Claude Extra High.
...
Pass 1 history is **`review.md`** (NOT CLEAN, `R7-F01`–`R7-F13`). Pass 2 history is **`review-rerun-2.md`** (NOT CLEAN, `R7b-F01`–`R7b-F17`; F01–F16 ACCEPT-applied; F17 REJECT). Pass 3 history is **`review-rerun-3.md`** (NOT CLEAN, `R7c-F01`–`R7c-F16`; all 16 ACCEPT-applied). Pass 4 history is **`review-rerun-4.md`** (NOT CLEAN, `R7d-F01`–`R7d-F12`; all 12 ACCEPT-applied). Do **not** overwrite `review.md`, `review-rerun-2.md`, `review-rerun-3.md`, or `review-rerun-4.md`. Write **`review-rerun-5.md`** only.

Issue ledger provided below (encoder `--write-review-brief` / Policy G pack); **do not re-report** those rows **including REJECT F17**. File **all valid residuals at this SHA, all severities including nits** (HIGH/MED/LOW/nit). New IDs **R7e-F01+** (no collision with `R7-F01`–`R7-F13`, `R7b-F01`–`R7b-F17`, `R7c-F01`–`R7c-F16`, or `R7d-F01`–`R7d-F12`). **CLEAN** only if nothing valid remains. ACCEPT items will APPLY as a pack. Reviewer is Claude High Pi (`claude/claude-opus-5-high`); Verify/Triage Composer 2.5; Fix Grok 4.6 High.
...
Ledger now includes **R7d-F01–R7d-F12** (ACCEPT / resolved yes / SHA `74b9acf2…`), **R7c-F01–R7c-F16** (ACCEPT / resolved yes / SHA `fce83948…`), **R7b-F01–R7b-F16** (ACCEPT / resolved yes / SHA `4c229f5d…`), **R7b-F17** (REJECT / resolved-as-rejected / not encoded), plus R7-F01–F13, R6b–R6n, and earlier. Pass 4 `review-rerun-4.md` is history on the **pre-APPLY** pin `fce83948…`. This pass is on the **post-APPLY** pin `74b9acf2…`. Do **not** overwrite Extra High files. Do **not** overwrite `review.md`, `review-rerun-2.md`, `review-rerun-3.md`, or `review-rerun-4.md`. Write **`review-rerun-5.md`** only in this Claude High work dir. Do **not** re-report REJECT F17.

## Why this pass exists

Claude High pass 4 (`review-rerun-4.md`) was **NOT CLEAN** (`R7d-F01`–`R7d-F12`). TRIAGE ACCEPT 12/12; REJECT none. APPLY packed F01–F12 into the freeze. `verify_1-apply-rerun-4.md` **PASS**. Launcher recorded `--record-rung-review-outcome accept-apply` → `consecutive_clean_reviews: 0` (required 2) for `rung-id` `rung-07-pi-claude-opus-5-high`. Policy F streak reset. This is **pass 5** — a fresh residual re-hunt on the **new** pin. REJECT does not break the streak; any new ACCEPT resets it after APPLY.

**Independent residual re-hunt is mandatory.** Do **not** rubber-stamp pass 1–4. Do **not** copy `review.md`, `review-rerun-2.md`, `review-rerun-3.md`, or `review-rerun-4.md`. Re-read the pinned freeze from scratch. Pass 1–4 are history, not authority. Residual only: do not re-file APPLYed / ledger IDs (including REJECT F17) unless a residual defect remains in **this** freeze text. File **all** valid residuals at this SHA, **all severities including nits** (HIGH/MED/LOW/nit). New IDs: `R7e-F01+`. If you find none, say **CLEAN** with evidence from **this** pass’s freeze read. CLEAN only if nothing valid remains. ACCEPT items will APPLY as a pack.

## Freeze (do not mutate)

- **File:** `.planning/spec_template_world_class.plan.md`
- **SHA-256:** `74b9acf23da1817834f35047c72bc1129fef4b2511ceac6c974fa5a8752fec33`
...
Hash both twins at start. They must still equal `74b9acf2…`. If they do not, stop and report. Do not edit them.
...
Write only:
`.planning/rfl-spec-template-world-class/rung-07-pi-claude-opus-5-high/review-rerun-5.md`
...
3. After writing `review-rerun-5.md`, `graphify update .`.
...
- Do NOT re-report ledger rows (including R7-F01–R7-F13, R7b-F01–F17, R7c-F01–F16, R7d-F01–F12, REJECT F17, and R6b–R6n).
- Do NOT overwrite `review.md`, `review-rerun-2.md`, `review-rerun-3.md`, or `review-rerun-4.md`.
