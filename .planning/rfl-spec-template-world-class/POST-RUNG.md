...
After **every** rung on [`.planning/rfl-spec-template-world-class/`](./), the launcher/parent (not the rung model) does this. Do not skip because the verdict was CLEAN. Do not dump raw `review.md`. Do not execute freeze YAML. Do not rewrite the freeze plan except ledger SHA metadata. Do not clobber `rung-NN-*/review.md` while a reviewer is writing it.

Skill: [`skills/silver-review-fix-ladder/SKILL.md`](../../skills/silver-review-fix-ladder/SKILL.md) Policy C + Policy F + Per-Rung Workflow.
...
2. **Policy C artifact-first** (`rung_N_triage`):
   ...
   - Encode:
     ...
     python3 scripts/review-fix-ladder.py --write-policy-c \
       --rung-dir .planning/rfl-spec-template-world-class/rung-NN-<slug>/ \
       --table-json-file .planning/rfl-spec-template-world-class/rung-NN-<slug>/policy-c-payload.json
...
6. **LADDER-STATUS.json** — keep `status: active` until the whole ladder finishes:
     ...
     python3 scripts/review-fix-ladder.py --mark-ladder-status active \
       --run-dir .planning/rfl-spec-template-world-class \
   ...
   - Then: `python3 scripts/review-fix-ladder.py --assert-rfl-advance --run-dir .planning/rfl-spec-template-world-class --rung-dir <rung> --next-action next_rung_review`
   - And: `python3 scripts/review-fix-ladder.py --assert-consecutive-clean --run-dir .planning/rfl-spec-template-world-class --rung-dir <rung>`
7. **Policy F consecutive CLEAN reviews (HARD)** — same model/effort until streak == 2:
   - After triage: if **zero ACCEPT** findings (CLEAN, or only REJECT): `python3 scripts/review-fix-ladder.py --record-rung-review-outcome clean --run-dir .planning/rfl-spec-template-world-class --rung-id <rung>`. If `consecutive_clean_reviews` < 2, **re-launch the same model** as review-only (keep original `review.md` as pass-N history; do not clobber). Do **not** start the next ladder model.
   - If **any ACCEPT**: verify_1/verify_2 then APPLY, then `--record-rung-review-outcome accept-apply` (streak **resets to 0**), then re-review the **same** model. Parent MUST NOT launch next model while `--assert-rfl-advance --next-action next_rung_review` fails.
8. **CHARTER.md** — last completed rung, current phase, freeze SHA pinned to APPLY.
9. **LADDER.md** — that row DONE (or SKIPPED/BLOCKED) only after Policy F streak == 2. Verify is **per rung**, not extra V1/V2 ladder rows.
10. Advance to N+1 **only** after `consecutive_clean_reviews == 2` **and** two clean verifies + greps + ACCEPTs applied (or REJECT-as-wrong recorded).
...
| Two consecutive CLEAN reviews | `LADDER-STATUS.json` `consecutive_clean_reviews` ≥ 2 | before next model (Policy F; SKIPPED.md exempt) |
