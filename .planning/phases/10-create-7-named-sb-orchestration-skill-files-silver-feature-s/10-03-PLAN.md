...
must_haves:
  truths:
    - "skills/silver-ui/SKILL.md exists with all 16 workflow steps"
    - "gsd-ui-phase replaces standard gsd-plan-phase for UI spec contract"
    - "gsd-ui-review (6-pillar visual audit) runs after execution"
    - "silver:tdd applied to component logic only"
  artifacts:
    - path: "skills/silver-ui/SKILL.md"
      provides: "UI/frontend orchestration skill"
      contains: "gsd-ui-phase"
  key_links:
    - from: "skills/silver-ui/SKILL.md"
      to: "gsd-ui-phase"
      via: "Skill tool invocation for UI spec contract"
      pattern: "gsd-ui-phase"
---

<objective>
Create `skills/silver-ui/SKILL.md` — SB orchestration skill for frontend, component, and interface work.

Purpose: Implements spec §4.3. Like silver:feature but diverges at execution: uses `gsd-ui-phase` to create a UI-SPEC.md design contract before planning, and adds `gsd-ui-review` (6-pillar visual audit) after execution. TDD applies to component logic only.

Output: `skills/silver-ui/SKILL.md` — thin orchestrator for the full UI development lifecycle.
</objective>
...
<task type="auto">
  <name>Task 1: Create skills/silver-ui/SKILL.md</name>
  <files>skills/silver-ui/SKILL.md</files>
  <action>
Create `skills/silver-ui/` directory and write `SKILL.md`:
...
grep -c "gsd-ui-phase\|gsd-ui-review\|silver:tdd\|silver:request-review\|silver:brainstorm" skills/silver-ui/SKILL.md
  ...
  <done>File exists, grep count ≥5, banner present, §10 prefs read, gsd-ui-phase (design contract step) present, gsd-ui-review (6-pillar visual audit) present, silver:tdd scoped to component logic, Steps 0–16 all present</done>
</task>

<task type="auto">
  <name>Task 2: Commit silver-ui skill</name>
  <files>skills/silver-ui/SKILL.md</files>
  <action>
```bash
...
```
  ...
  <verify>git log --oneline -1 | grep "silver:ui"</verify>
  <done>Commit exists with message "feat: add silver:ui orchestration skill"</done>
</task>
...
|----------|-------------|
| gsd-ui-phase output → gsd-plan-phase input | UI-SPEC.md design contract must exist before plan phase; skill enforces this ordering |

## STRIDE Threat Register

| Threat ID | Category | Component | Disposition | Mitigation Plan |
|-----------|----------|-----------|-------------|-----------------|
| T-10-06 | Information Disclosure | gsd-ui-review visual audit | accept | UI review is internal code review; no PII or secrets involved |
| T-10-07 | Spoofing | routing conflict (ui vs feature) | mitigate | Explicit note in skill: "if matches both ui and feature, ui wins" |
</threat_model>

<verification>
- `test -f skills/silver-ui/SKILL.md` → exists
- `grep -c "gsd-ui-phase\|gsd-ui-review\|silver:tdd\|silver:request-review\|silver:brainstorm" skills/silver-ui/SKILL.md` → ≥5
- `grep "SILVER BULLET ► UI WORKFLOW" skills/silver-ui/SKILL.md` → match
- `grep "§10" skills/silver-ui/SKILL.md` → match (prefs read)
- `git log --oneline -1 | grep "silver:ui"` → commit exists
</verification>

<success_criteria>
`skills/silver-ui/SKILL.md` exists with banner, §10 prefs, Steps 0–16, gsd-ui-phase for design contract, gsd-ui-review for visual audit, silver:tdd scoped to component logic, and all quality/review/ship steps present.
</success_criteria>

<output>
After completion, create `.planning/phases/10-create-7-named-sb-orchestration-skill-files-silver-feature-s/10-03-SUMMARY.md`
</output>
