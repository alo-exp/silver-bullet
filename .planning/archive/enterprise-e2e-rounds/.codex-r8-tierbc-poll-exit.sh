#!/usr/bin/env bash
# Poll codex-r8-tierbc driver until exit; full rescore + ledger + conditional FORCE/Phase C.
set -euo pipefail
...
INTERVAL="${2:-75}"
POLL_LOG="${ROOT}/.planning/enterprise-e2e/.codex-r8-tierbc-poll.log"
RESCORE_LOG="${ROOT}/.planning/enterprise-e2e/.codex-r8-tierbc-rescore.log"
MATRIX_LOG="${ROOT}/.e2e-matrix-codex-live.log"
LEDGER="${ROOT}/.planning/enterprise-e2e/ROUND-CODEX-1-LEDGER.md"
CHECKPOINT="${ROOT}/.planning/enterprise-e2e/.codex-r8-tierbc-checkpoint.md"
...
{
  printf '# codex-r8-tierbc checkpoint\n\n'
  printf -- '- **When:** %s *(EXIT)*\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  printf -- '- **Branch:** enterprise-e2e/codex @ %s\n' "$(git rev-parse --short HEAD 2>/dev/null || echo unknown)"
  printf -- '- **Driver PID:** %s EXITED\n' "$DRIVER_PID"
  printf -- '- **Rescore:** %s/22 — [%s](./.codex-r8-tierbc-rescore.log)\n' "$pass_count" ".codex-r8-tierbc-rescore.log"
  printf -- '- **Poll log:** [%s](./.codex-r8-tierbc-poll.log)\n' ".codex-r8-tierbc-poll.log"
} >"$CHECKPOINT"

# Append ledger poll checkpoint
{
  printf '\n### Poll checkpoint %s (tierbc exit — force4 follow-up)\n\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  printf '| Field | Value |\n|-------|-------|\n'
  printf '| **Driver** | EXITED PID **%s** |\n' "$DRIVER_PID"
  printf '| **Rescore** | **%s/22** — [.codex-r8-tierbc-rescore.log](./.codex-r8-tierbc-rescore.log) |\n' "$pass_count"
...
elif [[ "$pass_count" -eq 18 ]]; then
  fail15=0 fail16=0
  grep -q '^row 15 FAIL' "$RESCORE_LOG" && fail15=1
  grep -q '^row 16 FAIL' "$RESCORE_LOG" && fail16=1
  other_fails="$(grep '^row .* FAIL' "$RESCORE_LOG" | grep -vE '^row (15|16) ' || true)"
  if [[ "$fail15" -eq 1 && "$fail16" -eq 1 && -z "$other_fails" ]]; then
    log_line "DIAGNOSE rows 15,16 OUT-REVIEW-01/OUT-MEASURE-01 — launching single FORCE 15 16"
    export SB_ROOT="$ROOT"
    export SB_TEST_ENTERPRISE_APP_ROOT="${SB_TEST_ENTERPRISE_APP_ROOT:-/Users/shafqat/projects/enterprise-grade-test-app}"
    ...
    bash "${ROOT}/scripts/run-enterprise-e2e-live-test.sh" --skip-code-intel-preflight 15 16 \
      2>&1 | tee -a "$MATRIX_LOG" >>"$POLL_LOG" || true
    pass_count="$(run_full_rescore)"
    log_line "POST-FORCE-1516 rescore pass=${pass_count}/22"
  fi
fi
