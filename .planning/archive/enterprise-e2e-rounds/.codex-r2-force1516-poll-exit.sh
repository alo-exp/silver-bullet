#!/usr/bin/env bash
# Poll codex-r2-force1516 driver until exit; frozen-merge rescore from Tier C @ 71f93a3f.
set -euo pipefail
...
INTERVAL="${2:-75}"
POLL_LOG="${ROOT}/.planning/enterprise-e2e/.codex-r2-force1516-poll.log"
RESCORE_LOG="${ROOT}/.planning/enterprise-e2e/.codex-r2-force1516-rescore.log"
FROZEN_RESCORE_LOG="${ROOT}/.planning/enterprise-e2e/.codex-r2-matrix-rescore.log"
FROZEN_BASELINE_SHA="${SB_E2E_FROZEN_BASELINE_SHA:-71f93a3f}"
MATRIX_LOG="${ROOT}/.e2e-matrix-codex-live.log"
LEDGER="${ROOT}/.planning/enterprise-e2e/ROUND-CODEX-2-LEDGER.md"
CHECKPOINT="${ROOT}/.planning/enterprise-e2e/.codex-r2-force1516-checkpoint.md"
GATES="${ROOT}/.planning/enterprise-e2e/ROUND-CODEX-2-GATES.md"
...
run_full_rescore() {
  ...
  for row in $(seq 1 20); do
    if [[ "$row" -ne 15 && "$row" -ne 16 ]] && frozen_row_passes "$row"; then
...
{
  printf '# codex-r2-force1516 checkpoint\n\n'
  printf -- '- **When:** %s *(EXIT)*\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  printf -- '- **Branch:** enterprise-e2e/codex @ %s\n' "$(git rev-parse --short HEAD 2>/dev/null || echo unknown)"
  printf -- '- **Frozen baseline:** %s — rows 1–14, 17–22 PASS\n' "$FROZEN_BASELINE_SHA"
  printf -- '- **Driver PID:** %s EXITED\n' "$DRIVER_PID"
  printf -- '- **Rescore:** %s/22 — [.codex-r2-force1516-rescore.log](./.codex-r2-force1516-rescore.log)\n' "$pass_count"
  printf -- '- **Poll log:** [.codex-r2-force1516-poll.log](./.codex-r2-force1516-poll.log)\n'
} >"$CHECKPOINT"

{
  printf '\n### Poll checkpoint %s (force1516 exit)\n\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  printf '| Field | Value |\n|-------|-------|\n'
  printf '| **Policy** | One pass per row per SB SHA — frozen 1–14,17–22 @ `%s`; FORCE 15,16 live |\n' "$FROZEN_BASELINE_SHA"
...
if [[ "$pass_count" -eq 22 ]]; then
  ...
  if RTK_DISABLED=1 bash "${ROOT}/tests/run-all-tests.sh" >>"$POLL_LOG" 2>&1; then
    log_line "PHASE_C run-all-tests PASS"
    {
      printf '\n### Phase C complete %s\n\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
      printf '| Gate | Status |\n|------|--------|\n'
      printf '| Matrix ledger 22/22 | **PASS** |\n'
      printf '| Outcome assessment harness | **PASS** |\n'
      printf '| run-all-tests | **PASS** |\n'
      printf '| **2 consecutive strict clean rounds** | **PASS (2/2)** — Codex-1 + Codex-2 strict-clean |\n'
    } >>"$GATES"
    sed -i '' 's/^## Status: \*\*IN PROGRESS\*\*/## Status: **CLOSED Pass**/' "$GATES" 2>/dev/null || \
      sed -i 's/^## Status: \*\*IN PROGRESS\*\*/## Status: **CLOSED Pass**/' "$GATES"
  else
