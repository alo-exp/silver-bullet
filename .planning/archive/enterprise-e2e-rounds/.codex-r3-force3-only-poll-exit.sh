#!/usr/bin/env bash
# Poll codex-r3-force3-only driver until exit; frozen rows 1+6 PASS + rescore row 3 with §5b api/currency gate.
set -euo pipefail
...
CHECKPOINT="${ROOT}/.planning/enterprise-e2e/.codex-r3-force3-only-checkpoint.md"
...
frozen_row6_passes() {
  ...
  export SB_E2E_ROW6_FROZEN_COMMIT="$ROW6_FROZEN"
  # shellcheck source=scripts/lib/enterprise-e2e-live-common.sh
  source "${ROOT}/scripts/lib/enterprise-e2e-live-common.sh"
  local row_log
  row_log="$(latest_row_log 6)"
  if [[ -n "$row_log" && -f "$row_log" ]]; then
    ...
    if enterprise_e2e_outcome_row_passes 6 "$SB_TEST_ENTERPRISE_APP_ROOT" "$STATE_DIR" \
      "$row_log" "$LEDGER" ".planning/workflows/fast-readme.md"; then
      return 0
    fi
  fi
  # Frozen by policy when fixture pinned @ ROW6_FROZEN and prior force3 row6 PASS.
  if [[ -f "$TIERB_RESCORE_LOG" ]] && grep -qE '^row 6 PASS' "$TIERB_RESCORE_LOG"; then
    return 0
  fi
  enterprise_e2e_assert_row_product_commit_rescore 6 "$SB_TEST_ENTERPRISE_APP_ROOT"
}
...
run_tierb_rescore() {
  ...
  export SB_E2E_TEST_APP_BASELINE_SHA="${SB_E2E_TEST_APP_BASELINE_SHA:-09f8d1a}"
  export SB_E2E_ROW6_FROZEN_COMMIT="$ROW6_FROZEN"
  export SB_E2E_ROW3_PRODUCT_ANCHOR_SHA="$ROW6_FROZEN"
  # shellcheck source=scripts/lib/enterprise-e2e-live-common.sh
  source "${ROOT}/scripts/lib/enterprise-e2e-live-common.sh"
  # shellcheck source=scripts/enterprise-e2e/lib/deterministic/outcome-assessment.sh
  source "${ROOT}/scripts/enterprise-e2e/lib/deterministic/outcome-assessment.sh"
  ...
  if frozen_row6_passes; then
    printf 'row 6 PASS (frozen@%s)\n' "$ROW6_FROZEN" >>"$RESCORE_LOG"
    pass=$((pass + 1))
  else
    printf 'row 6 FAIL (frozen check)\n' >>"$RESCORE_LOG"
    fail_rows+=(6)
  fi

  row=3
  evidence=".planning/workflows/feature-currency.md"
  row_log="$(latest_row_log "$row")"
  if [[ -z "$row_log" || ! -f "$row_log" ]]; then
    printf 'row %d SKIP no log\n' "$row" >>"$RESCORE_LOG"
    fail_rows+=("$row")
  elif ! enterprise_e2e_assert_row_product_commit_rescore "$row" "$WORK_DIR" >>"$RESCORE_LOG" 2>&1; then
    printf 'row %d FAIL: §5b api/currency commit\n' "$row" >>"$RESCORE_LOG"
    fail_rows+=("$row")
  elif enterprise_e2e_outcome_row_passes "$row" "$WORK_DIR" "$STATE_DIR" "$row_log" "$LEDGER" "$evidence"; then
    printf 'row %d PASS\n' "$row" >>"$RESCORE_LOG"
    pass=$((pass + 1))
  else
    local fails=""
  ...
  cp "$RESCORE_LOG" "$TIERB_RESCORE_LOG"
  printf '%d\n' "$pass"
}

log_line "POLL-EXIT start driver=${DRIVER_PID} interval=${INTERVAL}s branch=$(git rev-parse --short HEAD 2>/dev/null || echo unknown) frozen_row1@${FROZEN_SHA} frozen_row6@${ROW6_FROZEN}"
resume_friction_monitor
...
{
  printf '# codex-r3-force3-only checkpoint\n\n'
  printf -- '- **When:** %s *(EXIT)*\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  printf -- '- **Branch:** enterprise-e2e/codex @ %s\n' "$(git rev-parse --short HEAD 2>/dev/null || echo unknown)"
  printf -- '- **Frozen row 1:** PASS @ `%s`\n' "$FROZEN_SHA"
  printf -- '- **Frozen row 6:** PASS @ `%s`\n' "$ROW6_FROZEN"
  printf -- '- **Driver PID:** %s EXITED\n' "$DRIVER_PID"
  printf -- '- **Tier B rescore:** %s/3 — [.codex-r3-force3-only-rescore.log](./.codex-r3-force3-only-rescore.log)\n' "$pass_count"
} >"$CHECKPOINT"

{
  printf '\n### Poll checkpoint %s (force3-only exit @ %s)\n\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$(git rev-parse --short HEAD 2>/dev/null || echo unknown)"
  printf '| Field | Value |\n|-------|-------|\n'
  printf '| **Policy** | One pass — frozen rows **1+6** PASS; FORCE row **3** only; fixture @ `%s` |\n' "$ROW6_FROZEN"
