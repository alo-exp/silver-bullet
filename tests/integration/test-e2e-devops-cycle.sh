...
# Tests the devops-cycle active_workflow — silver-blast-radius + devops-quality-gates replace silver-quality-gates
set -euo pipefail
...
echo "--- Scenario 1: DevOps planning gate (silver-blast-radius + devops-quality-gates) ---"
integration_setup
write_devops_config

# No skills: blocked (silver-blast-radius missing — Stage A)
...
assert_contains "S1.2: mentions silver-blast-radius" "$out" "silver-blast-radius"

# Record silver-blast-radius only: still blocked (devops-quality-gates missing)
...
# Scenario 2: DevOps commit gate requires silver-blast-radius
echo "--- Scenario 2: DevOps commit gate ---"
integration_setup
write_devops_config

# No silver-blast-radius: commit blocked
out=$(run_completion_audit "PreToolUse" "git commit -m 'infra: update deploy'")
assert_blocked "S2.1: commit blocked without silver-blast-radius" "$out"
...
# Only silver-blast-radius: PR blocked
run_record_skill "silver-blast-radius" >/dev/null
