...
printf '%s' "$ctx" | grep 'CONSENT PENDING' >/dev/null && pass "pending consent injects prompt" || fail "pending consent injects prompt"

write_consent false
out="$(run_session)"
ctx="$(printf '%s' "$out" | extract_ctx)"
printf '%s' "$ctx" | grep 'opted out' >/dev/null && pass "disabled consent injects opted-out note" || fail "disabled consent injects opted-out note"

write_consent true
out="$(run_session)"
ctx="$(printf '%s' "$out" | extract_ctx)"
# enabled + no CLI should mention install
printf '%s' "$ctx" | grep -i 'graphify' >/dev/null && pass "enabled consent injects graphify status" || fail "enabled consent injects graphify status"

write_consent true true
out="$(run_session)"
ctx="$(printf '%s' "$out" | extract_ctx)"
printf '%s' "$ctx" | grep 'enforcement suspended' >/dev/null && pass "suspended opt-in injects suspension note" || fail "suspended opt-in injects suspension note"
...
out="$(run_session)"
ctx="$(printf '%s' "$out" | extract_ctx)"
printf '%s' "$ctx" | grep 'agentmemory' >/dev/null && pass "agentmemory pending consent injects prompt" || fail "agentmemory pending consent injects prompt"
...
out="$(run_session)"
ctx="$(printf '%s' "$out" | extract_ctx)"
printf '%s' "$ctx" | grep -i 'rtk' >/dev/null && pass "rtk pending consent injects prompt" || fail "rtk pending consent injects prompt"
...
out="$(run_session)"
ctx="$(printf '%s' "$out" | extract_ctx)"
printf '%s' "$ctx" | grep 'verification only' >/dev/null && pass "session-start injects verification-only header" || fail "session-start injects verification-only header"
