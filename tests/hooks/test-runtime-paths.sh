...
for runtime in claude codex cursor; do
  out=$(env -u SB_RUNTIME_HOME_ROOT SILVER_BULLET_RUNTIME="$runtime" HOME="$TMP_HOME" bash -c "source '$RUNTIME_PATHS'; printf '%s|%s' \"\$SILVER_BULLET_RUNTIME\" \"\$SB_RUNTIME_STATE_DIR\"")
...
out=$(env -u SILVER_BULLET_RUNTIME HOME="$TMP_HOME" bash -c 'source "'"$TMP_HOME"'/.cursor/plugins/cache/alo-labs/silver-bullet/0.0.0/hooks/lib/runtime-paths.sh"; printf "%s|%s" "$SILVER_BULLET_RUNTIME" "$SB_RUNTIME_STATE_DIR"')
