#!/usr/bin/env bash
# Tests for hooks/record-agentmemory-usage.sh
set -euo pipefail
...
cleanup_all() {
  rm -rf "$TMPDIR_TEST" "${SB_TEST_DIR}/agentmemory-usage-${TEST_RUN_ID}"
}
...
echo "=== record-agentmemory-usage.sh tests ==="

setup
run_bash_hook 'curl -s -X POST http://localhost:3111/agentmemory/smart-search -H "Content-Type: application/json" -d "{\"query\":\"test\"}"' 0
if [[ -f "$AM_STATE" ]] && grep -qE '^[0-9]+$' "$AM_STATE"; then
  pass "records epoch on successful smart-search"
else
  fail "records epoch on successful smart-search"
fi

setup
run_bash_hook 'curl -s -X POST http://localhost:3111/agentmemory/smart-search -d "{}"' 1
if [[ ! -f "$AM_STATE" ]]; then
...
if [[ ! -f "$AM_STATE" ]]; then
  pass "ignores non-agentmemory commands"
else
  fail "ignores non-agentmemory commands"
fi
...
EOF
run_bash_hook 'curl -s http://localhost:3111/agentmemory/smart-search' 0
if [[ ! -f "$AM_STATE" ]]; then
