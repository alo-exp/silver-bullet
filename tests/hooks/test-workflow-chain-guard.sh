...
if grep -q 'sb-\*) composer="silver-' "$REPO_ROOT/hooks/workflow-chain-guard.sh"; then
  echo "  PASS: public sb workflow slugs normalize to canonical policy slugs"
  PASS=$((PASS + 1))
else
  echo "  FAIL: public sb workflow slugs normalize to canonical policy slugs"
  FAIL=$((FAIL + 1))
fi
...
start_workflow "/sb:feature" "feature gate test" "bootstrap,design,execute,verify"
out=$(run_hook_edit "$TMPDIR_TEST/src/app.js")
assert_blocks "sb:feature blocks without SB pre-execution markers" "$out"
write_state_markers silver-quality-gates silver-context silver-plan silver-spec silver-validate
out=$(run_hook_edit "$TMPDIR_TEST/src/app.js")
assert_passes "sb:feature passes after SB-owned pre-execution markers exist" "$out"
write_state_markers silver-quality-gates silver-context silver-plan silver-validate
out=$(run_hook_edit "$TMPDIR_TEST/src/app.js")
assert_passes "sb:feature legacy markers satisfy SB-owned aliases" "$out"
teardown

# UI workflow: UI-specific pre-execution SB markers must be present.
setup
touch "$TMPDIR_TEST/src/app.js"
start_workflow "/sb:ui" "ui gate test" "orient,design,plan,execute,review,verify"
write_state_markers silver-context silver-ui-contract silver-plan
out=$(run_hook_edit "$TMPDIR_TEST/src/app.js")
assert_blocks "sb:ui blocks until UI pre-execution markers are present" "$out"
write_state_markers silver-quality-gates silver-context silver-plan silver-ui-contract silver-spec silver-validate
out=$(run_hook_edit "$TMPDIR_TEST/src/app.js")
assert_passes "sb:ui passes after SB-owned UI pre-execution markers exist" "$out"
write_state_markers silver-quality-gates silver-context silver-ui-contract silver-plan silver-validate
out=$(run_hook_edit "$TMPDIR_TEST/src/app.js")
assert_passes "sb:ui legacy UI markers satisfy SB-owned aliases" "$out"
teardown

# Deep research workflow: clarify + deep-research markers are required.
setup
touch "$TMPDIR_TEST/src/app.js"
start_workflow "/sb:deep-research" "research gate test" "clarify,research,hand-off"
out=$(run_hook_edit "$TMPDIR_TEST/src/app.js")
assert_blocks "sb:deep-research blocks without clarify marker" "$out"
write_state_markers silver-clarify
out=$(run_hook_edit "$TMPDIR_TEST/src/app.js")
assert_blocks "sb:deep-research blocks with only clarify marker (research missing)" "$out"
write_state_markers silver-clarify silver-deep-research
out=$(run_hook_edit "$TMPDIR_TEST/src/app.js")
assert_passes "sb:deep-research passes after clarify + research markers exist" "$out"
teardown

# Multiple active workflows: SB_WORKFLOW_ID scopes the guard to one workflow file.
setup
touch "$TMPDIR_TEST/src/app.js"
wf_a=$( cd "$TMPDIR_TEST" && bash "$WORKFLOWS_SCRIPT" start "/sb:feature" "wf-a" "plan,execute" )
wf_b=$( cd "$TMPDIR_TEST" && bash "$WORKFLOWS_SCRIPT" start "/sb:bugfix" "wf-b" "debug,plan" )
write_state_markers silver-quality-gates silver-context silver-plan silver-spec silver-validate
...
setup
touch "$TMPDIR_TEST/src/app.js"
start_workflow "/sb:fast" "fast gate test" "context,plan,execute,verify"
out=$(run_hook_edit "$TMPDIR_TEST/src/app.js")
assert_blocks "sb:fast blocks without quality-gates/plan/validate markers" "$out"
write_state_markers silver-quality-gates-design silver-plan silver-validate
out=$(run_hook_edit "$TMPDIR_TEST/src/app.js")
assert_passes "sb:fast passes after Tier 2 pre-execution markers exist" "$out"
teardown
...
touch "$TMPDIR_TEST/src/app.js"
out=$(run_hook_edit "$TMPDIR_TEST/src/app.js")
assert_passes "sb:fast Tier 1 without workflow tracker does not block edits" "$out"
teardown
...
start_workflow "/sb:devops" "devops gate test" "blast-radius,plan,execute,verify"
out=$(run_hook_edit "$TMPDIR_TEST/infra.tf")
assert_blocks "sb:devops blocks without pre-execution markers" "$out"
...
out=$(run_hook_edit "$TMPDIR_TEST/infra.tf")
assert_passes "sb:devops passes after full pre-execution chain (no SPEC — plan-only validate)" "$out"
teardown
...
touch "$TMPDIR_TEST/src/app.js"
rm -f "$TMPDIR_TEST/.planning/SPEC.md"
start_workflow "/sb:feature" "feature no spec" "plan,execute"
write_state_markers silver-quality-gates silver-context silver-plan silver-validate
out=$(run_hook_edit "$TMPDIR_TEST/src/app.js")
assert_blocks "sb:feature without SPEC.md blocks until clarify+spec recorded" "$out"
write_state_markers silver-quality-gates silver-context silver-plan silver-spec silver-validate
out=$(run_hook_edit "$TMPDIR_TEST/src/app.js")
assert_blocks "sb:feature without SPEC.md still blocks with spec but no clarify" "$out"
write_state_markers silver-quality-gates silver-context silver-plan silver-clarify silver-spec silver-validate
out=$(run_hook_edit "$TMPDIR_TEST/src/app.js")
assert_passes "sb:feature without SPEC.md passes after clarify+spec markers" "$out"
teardown

# Feature with existing SPEC.md skips silver-spec requirement.
setup
touch "$TMPDIR_TEST/src/app.js"
start_workflow "/sb:feature" "feature with spec" "plan,execute"
write_state_markers silver-quality-gates silver-context silver-plan silver-validate
out=$(run_hook_edit "$TMPDIR_TEST/src/app.js")
assert_passes "sb:feature with SPEC.md skips silver-spec marker" "$out"
teardown
...
touch "$TMPDIR_TEST/src/app.js"
rm -f "$TMPDIR_TEST/.planning/SPEC.md"
start_workflow "/sb:ui" "ui no spec" "plan,design,execute"
write_state_markers silver-quality-gates silver-context silver-plan silver-ui-contract silver-validate
out=$(run_hook_edit "$TMPDIR_TEST/src/app.js")
assert_blocks "sb:ui without SPEC.md blocks until clarify+spec recorded" "$out"
write_state_markers silver-quality-gates silver-context silver-plan silver-ui-contract silver-spec silver-validate
out=$(run_hook_edit "$TMPDIR_TEST/src/app.js")
assert_blocks "sb:ui without SPEC.md still blocks with spec but no clarify" "$out"
...
out=$(run_hook_edit "$TMPDIR_TEST/src/app.js")
assert_passes "sb:ui without SPEC.md passes after clarify+spec markers" "$out"
teardown
...
setup
touch "$TMPDIR_TEST/src/app.js"
start_workflow "/sb:bugfix" "bugfix gate test" "orient,debug,plan,execute,review,verify"
out=$(run_hook_edit "$TMPDIR_TEST/src/app.js")
assert_blocks "sb:bugfix blocks without DEBUG/PLAN markers" "$out"
write_state_markers silver-debug
out=$(run_hook_edit "$TMPDIR_TEST/src/app.js")
assert_blocks "sb:bugfix still blocks with only DEBUG marker (PLAN missing)" "$out"
write_state_markers silver-debug silver-plan
out=$(run_hook_edit "$TMPDIR_TEST/src/app.js")
assert_passes "sb:bugfix passes after DEBUG + PLAN markers exist" "$out"
write_state_markers silver-debug silver-plan
out=$(run_hook_edit "$TMPDIR_TEST/src/app.js")
assert_passes "sb:bugfix legacy GSD debug/plan markers satisfy SB-owned aliases" "$out"
teardown
...
start_workflow "/sb:release" "release gate test" "quality-gate,audit,ship,create-release"
out=$(run_hook_edit "$TMPDIR_TEST/docs/CHANGELOG.md")
assert_blocks "sb:release blocks without silver-quality-gates marker" "$out"
write_state_markers silver-quality-gates-design
out=$(run_hook_edit "$TMPDIR_TEST/docs/CHANGELOG.md")
assert_passes "sb:release passes after pre-plan quality-gates marker exists" "$out"
teardown

# apply_patch must honor the same pre-execution chain as Edit (Codex parity).
setup
touch "$TMPDIR_TEST/src/app.js"
start_workflow "/sb:feature" "apply_patch gate" "plan,execute"
...
setup
touch "$TMPDIR_TEST/src/app.js"
start_workflow "/sb:feature" "post-exec only gate" "plan,execute"
write_state_markers silver-execute silver-review-request silver-verify security silver-ship
out=$(run_hook_edit "$TMPDIR_TEST/src/app.js")
assert_blocks "sb:feature blocks when only post-exec markers recorded" "$out"
teardown

# Pre-exec markers without post-exec markers must allow edits.
setup
touch "$TMPDIR_TEST/src/app.js"
start_workflow "/sb:feature" "pre-exec only gate" "plan,execute"
write_state_markers silver-quality-gates silver-context silver-plan silver-spec silver-validate
out=$(run_hook_edit "$TMPDIR_TEST/src/app.js")
assert_passes "sb:feature passes with pre-exec markers only (post-exec absent)" "$out"
teardown

# Only silver-execute recorded (plan missing) must block.
setup
touch "$TMPDIR_TEST/src/app.js"
start_workflow "/sb:feature" "execute without plan" "plan,execute"
write_state_markers silver-execute
out=$(run_hook_edit "$TMPDIR_TEST/src/app.js")
assert_blocks "sb:feature blocks with only silver-execute marker (plan missing)" "$out"
teardown
