...
assert_file_contains "native codex agent enables hook trust bypass for isolated runs" "$REPO_ROOT/tests/live/agents/codex/agent.sh" 'CODEX_BYPASS_HOOK_TRUST="$hook_trust_bypass"'
assert_file_contains "native codex agent enables hook auto-trust for isolated runs" "$REPO_ROOT/tests/live/agents/codex/agent.sh" 'CODEX_AUTO_TRUST_HOOKS="$auto_trust_hooks"'
