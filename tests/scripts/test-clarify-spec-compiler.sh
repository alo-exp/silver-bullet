...
assert_contains "spec routes to clarify --spec when no brief" 'sb:clarify --spec' "$SPEC"
...
# --- ingest next-step ---
assert_contains "ingest next-step is clarify then spec" "sb:clarify --spec \(or --next spec\) then /sb:spec" "$INGEST"
...
assert_contains "SPECIFY worker runs clarify --spec first when brief missing" "sb:clarify --spec" "$SPECIFY"
assert_not_contains "SPECIFY worker does not run Turns 1–9 inside spec" "Run 9 questioning turns" "$SPECIFY"
assert_contains "CLARIFY worker documents next=spec" "sb:clarify --spec" "$CLARIFY_W"
assert_contains "CLARIFY worker keeps light FLOW 3 default" "light FLOW 3" "$CLARIFY_W"

# --- live instructions ---
assert_contains "Spec Lifecycle Create uses clarify then spec" "sb:clarify --spec" "$SB"
assert_contains "Spec Lifecycle names compile" "compile canonical artifacts" "$SB"
assert_not_contains "Spec Lifecycle does not call spec Socratic elicitation" "sb:spec. \(Socratic elicitation\)" "$SB"
...
assert_contains "help ingest next is clarify then spec" "sb:clarify --spec" "$HELP_INGEST"


# --- composition skip (feature/UI without SPEC.md) ---
assert_contains "feature FLOW 3 --spec is mandatory when SPEC.md absent" "FLOW 3 = \`sb:clarify --spec\` is mandatory" "$FEATURE"
assert_contains "feature enforcement queue is clarify --spec then spec" "sb:clarify --spec\` then \`sb:spec" "$FEATURE"
assert_not_contains "feature does not treat no-SPEC as FLOW 5 only" "No \`.planning/SPEC.md\` \| FLOW 5 \(SPECIFY\) — enforced" "$FEATURE"
assert_contains "ui FLOW 3 --spec is mandatory when SPEC.md absent" "FLOW 3 = \`sb:clarify --spec\` is mandatory" "$UI"
assert_contains "ui enforcement queue is clarify --spec then spec" "sb:clarify --spec\` then \`sb:spec" "$UI"
...
assert_contains "router write-a-spec goes clarify --spec then spec" "sb:clarify --spec\` then \`sb:spec" "$SILVER"
...
assert_contains "runtime CLARIFY worker restamped with next=spec" "sb:clarify --spec" "$RUNTIME_CLARIFY"
