...
assert_path_absent "Claude progressive review loop does not expose OpenAI metadata" "$REPO_ROOT/agents/claude/progressive-review-loop/agents/openai.yaml"
...
assert_contains "Silver router installs missing dependency skills before fallback" "follow the missing dependency protocol before any fallback" "$(skill_file silver)"
...
assert_contains "Silver spec repairs Product Management dependency before fallback" "first repair the missing dependency from its marketplace source" "$(skill_file silver-spec)"
assert_contains "Silver spec documents Product Management marketplace install source" "/plugin install anthropics/knowledge-work-plugins/tree/main/product-management" "$(skill_file silver-spec)"
