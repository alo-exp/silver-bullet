...
if [[ -d "${REPO_ROOT}/commands" ]]; then
  fail "repo-root commands/ should not exist (Cursor command surface lives in plugins/silver-bullet/commands)"
else
  pass "repo-root commands/ is absent"
fi

if [[ -d "${REPO_ROOT}/plugins/silver-bullet/commands" ]]; then
  pass "Cursor package commands directory exists"
else
  fail "Cursor package commands directory exists"
fi
...
if grep -qE '^name: sb:feature$' "${REPO_ROOT}/agents/claude/sb:feature/SKILL.md"; then
  pass "Claude bundle exposes sb:feature skill name"
else
  fail "Claude bundle exposes sb:feature skill name"
fi

hyphen_skill_dirs="$(find "${REPO_ROOT}/agents/claude" -mindepth 1 -maxdepth 1 -type d -name 'sb-*' 2>/dev/null | wc -l | tr -d ' ')"
if [[ "$hyphen_skill_dirs" == "0" ]]; then
  pass "Claude bundle has no sb-* hyphen skill directories"
else
  fail "Claude bundle has no sb-* hyphen skill directories — found $hyphen_skill_dirs"
fi
...
while IFS= read -r skill_file; do
  ...
  if [[ "$invocable" != "false" ]]; then
    ...
    fail "Claude bundle skill without sb prefix is user-invocable: $name"
  fi
done < <(find "${REPO_ROOT}/agents/claude" -name SKILL.md -type f)

if [[ "$non_silver_user_invocable" -eq 0 ]]; then
  pass "Claude bundle hides non-sb helper skills from picker"
fi
