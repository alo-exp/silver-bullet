...
1. Detect the active host instruction file per `docs/RUNTIME-COMPATIBILITY.md`:
   ...
   - Cursor / task host → `AGENTS.md` when present
...
7. Do not create `primary host.md` on Cursor-only projects or `AGENTS.md` on primary host-only projects unless the user explicitly requests it.
...
Re-register SB hooks in the active host settings file (same contract as `/silver:init` update mode §3.7.5). Resolve `INSTALL_PATH` from `$HOME/.cursor/plugins/installed_plugins.json` when possible; fall back to `PLUGIN_ROOT`.

Per-host merge scripts live under `scripts/lib/install-<runtime>/` — see `docs/RUNTIME-COMPATIBILITY.md` (`merge-cursor-hooks.py`, `merge-cursor-hooks.py`).
...
if [[ -f "${PLUGIN_ROOT}/scripts/lib/install-cursor/merge-cursor-hooks.py" ]]; then
  ...
  python3 "${PLUGIN_ROOT}/scripts/lib/install-primary host/merge-cursor-hooks.py" "$INSTALL_PATH" 2>/dev/null || true
fi
```
...
if [[ -x scripts/workflows.sh ]]; then
  ...
  SB_WORKFLOWS_BIN="$(
    for root in \
      ...
      "$HOME/.cursor/plugins/cache/alo-labs/silver-bullet/current" \
      "$HOME/.cursor/plugins/cache/alo-labs/silver-bullet"/* \
...
- Canonical skill text stays host-neutral; run `python3 scripts/render-agent-bundle.py render --agent {primary host,Cursor,task host} --source-root skills --dest-root agents/<agent>` and `bash scripts/sync-Cursor-package.sh` after editing this file in the SB source repo.
