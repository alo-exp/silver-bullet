#!/usr/bin/env bash
# Channel-timeline monitor for Pi delegation logs (parent supervision).
set -euo pipefail

LOG_FILE=""
INTERVAL="${AGENT_PI_MONITOR_INTERVAL:-30}"
STALL_SEC="${AGENT_PI_MONITOR_STALL_SEC:-300}"
ONCE=0
...
scan_log() {
  ...
  if [[ "$prompt_seen" -eq 0 ]] && grep -qE 'prompt\.submitted|Prompt submitted|Submitted prompt|pi -p' <<<"$content" 2>/dev/null; then
  ...
  floor="${SB_AGENT_PI_LOG_FLOOR:-512}"
