#!/usr/bin/env bash
# Invoke wrapper: preflight → env → agent-pi-delegate.sh
set -euo pipefail
...
DELEGATE="${REPO_ROOT}/scripts/agent-pi-delegate.sh"
SKIP_PREFLIGHT=0
DELEGATE_ARGS=()

usage() {
  cat <<'EOF'
Usage: invoke.sh [--skip-preflight] <agent-pi-delegate.sh args>

Runs preflight + env defaults, then delegates to scripts/agent-pi-delegate.sh.
