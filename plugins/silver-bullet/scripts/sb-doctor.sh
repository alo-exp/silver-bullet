...
run_doctor_checks() {
  ...
  if [[ "$runtime" == "cursor" ]]; then
    ...
    if [[ -n "$backend_sha" && "$backend_sha" != "null" ]]; then
      gitpath_root="${HOME}/.codex/plugins/marketplaces/github.com/alo-exp/silver-bullet/${backend_sha}"
      market_cache_link="${HOME}/.codex/plugins/cache/alo-labs-cursor/silver-bullet/${backend_sha}"
      backend_cache_path="${HOME}/.codex/plugins/cache/alo-labs-agent-plugins/silver-bullet/${backend_sha}"
  ...
  if [[ "$runtime" == "cursor" ]]; then
    ...
    if [[ "$csba_check" -eq 0 ]]; then
      ...
      if [[ "$csba_scope" == "project" ]]; then
        ...
        csba_agents_dir="${HOME}/.codex/agents"
      fi
