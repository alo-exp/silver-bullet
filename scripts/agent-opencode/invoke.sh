#!/usr/bin/env bash
# Invoke wrapper: preflight → env → agent-opencode-delegate.sh
set -euo pipefail
...
DELEGATE="${REPO_ROOT}/scripts/agent-opencode-delegate.sh"
SKIP_PREFLIGHT=0
DELEGATE_ARGS=()

usage() {
  cat <<'EOF'
Usage: invoke.sh [--skip-preflight] <agent-opencode-delegate.sh args>

Runs preflight + env defaults, then delegates to scripts/agent-opencode-delegate.sh.
All delegate flags (--work-dir, --brief-file, etc.) pass through.
EOF
}
