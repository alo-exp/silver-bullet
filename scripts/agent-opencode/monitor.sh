#!/usr/bin/env bash
# Channel-timeline monitor for OpenCode delegation logs (parent supervision).
set -euo pipefail

LOG_FILE=""
INTERVAL="${AGENT_OPENCODE_MONITOR_INTERVAL:-30}"
STALL_SEC="${AGENT_OPENCODE_MONITOR_STALL_SEC:-300}"
ONCE=0
...
emit_bullet() {
  printf '[agent-opencode monitor] %s — %s\n' "$(date -u +%H:%M:%SZ)" "$1"
}
...
scan_log() {
  ...
  if [[ "$prompt_seen" -eq 0 ]] && grep -qE 'prompt\.submitted|Prompt submitted|Submitted prompt|opencode run' <<<"$content" 2>/dev/null; then
  ...
  if grep -qiE 'ERROR:|0-token|rate limit|429|token plan|not logged in|auth' <<<"$content"; then
    emit_bullet "• signal: $(grep -oiE 'ERROR:[^[:space:]]+|0-token|rate limit|429|token plan|not logged in' <<<"$content" | tail -1 || echo anomaly)"
  fi

  floor="${SB_AGENT_OPENCODE_LOG_FLOOR:-512}"
