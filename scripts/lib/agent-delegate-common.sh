#!/usr/bin/env bash
# Shared delegate wrapper behavior for agent-cursor-delegate.sh and agent-claude-delegate.sh.
...
agent_delegate_normalize_failure_class() {
  ...
  if [[ -n "$log_file" && -f "$log_file" ]]; then
    local log_bytes
    log_bytes="$(wc -c <"$log_file" | tr -d ' ')"
    local floor="${SB_AGENT_CLAUDE_LOG_FLOOR:-${SB_AGENT_CODEX_LOG_FLOOR:-${SB_AGENT_DELEGATE_LOG_FLOOR:-512}}}"
