...
sb_orchestrator_worker_template_for_skill() {
  ...
  case "$skill" in
    ...
    silver-agent-cursor|silver-agent-claude) printf 'AGENT-DELEGATE' ;;
    *)
...
sb_orchestrator_parent_delegate_bash_allowed() {
  ...
  if ! printf '%s' "$command_str" | grep -qE '(^|[[:space:]/])(\./)?scripts/agent-(cursor|claude)-delegate\.sh([[:space:]]|$)|(^|[[:space:]/])(\./)?scripts/agent-claude/invoke\.sh([[:space:]]|$)'; then
    return 1
  fi
...
sb_orchestrator_parent_skill_allowed() {
  ...
  case "$canonical" in
    silver|silver-orchestrator|silver-agent-cursor|silver-agent-claude) return 0 ;;
    *) return 1 ;;
  esac
}
