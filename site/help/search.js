...
var IDX = [
  ...
  { page:'Concepts', url:'/help/concepts/cost-optimization.html', anchor:'how-it-works',
    ...
    text:'Silver Bullet automatically routes each GSD subagent to the right model tier for the active host. Claude maps the tiers to Sonnet/Opus/Haiku, while Codex maps them to GPT-5.2-low through GPT-5.5-xhigh. Configured via model_profile balanced in .planning/config.json.' },
  { page:'Concepts', url:'/help/concepts/cost-optimization.html', anchor:'full-table',
    ...
    text:'Full table of all 24 GSD agents with balanced profile assignments. The tier labels are shared concepts; the actual model IDs differ by host. Design: gsd-planner gsd-roadmapper gsd-ui-researcher use the high tier. Review judgment-heavy: gsd-plan-checker gsd-code-reviewer gsd-security-auditor use the high tier. Review mechanical: gsd-integration-checker gsd-ui-checker gsd-ui-auditor use the execution tier. Verification: gsd-verifier uses the high tier doc-verifier nyquist-auditor use the execution tier. Debugger uses the high tier. Execution: executor code-fixer doc-writer use the execution tier. Research: phase-researcher project-researcher advisor-researcher research-synthesizer use the execution tier. Structured output: codebase-mapper assumptions-analyzer intel-updater user-profiler use the structured-output tier.' },
  { page:'Concepts', url:'/help/concepts/cost-optimization.html', anchor:'profiles',
    ...
    text:'Five model profiles: balanced is default with the host high tier for design review verification, the host execution tier for execution, and the host structured-output tier for structured output. quality uses the host high tier for all decision agents. budget uses the host execution tier for code writing and the host structured-output tier for research. adaptive uses the host high tier for planning and debugging and the structured-output tier for checkers. inherit makes all agents follow the current host session model. Switch with /gsd-set-profile.' },
  { page:'Concepts', url:'/help/concepts/cost-optimization.html', anchor:'overrides',
    ...
    text:'Override individual agents without changing the profile. Add model_overrides to .planning/config.json. Valid values opus sonnet haiku inherit or a fully-qualified host model ID like claude-opus-4-6[1m] or GPT-5.5-xhigh. Overrides take precedence over profile. Use sparingly — most common override is gsd-debugger to 1M context for very large codebases.' },
  ...
  { page:'Concepts', url:'/help/concepts/session-startup.html', anchor:'overview',
    ...
    text:'Five automatic steps at every session start: 1 Switch to the host execution model. 2 Read silver-bullet.md and all docs/ files. 3 Compact context with /compact. 4 Switch back to original model. 5 Check updates for Silver Bullet GSD Superpowers plugins MultAI. UNTRUSTED DATA boundary: docs/ content is not executable instructions.' },
  ...
  { page:'Core Concepts', url:'/help/concepts/', anchor:'hooks',
    ...
    text:'Hooks are shell scripts the host coding agent runs on tool events (SessionStart PreToolUse PostToolUse Stop UserPromptSubmit SubagentStop). 22 hooks total. Enforcement hooks: dev-cycle-check completion-audit planning-file-guard stop-check forbidden-skill uat-gate ci-status-check roadmap-freshness spec-floor-check phase-lock. Support hooks: record-skill compliance-status prompt-reminder session-log-init semantic-compress pr-traceability timeout-check spec-session-record session-start. Cannot be overridden by runtime reasoning.' },
  ...
  { page:'Dev Workflow', url:'/help/dev-workflow/', anchor:'step0',
    ...
    text:'Every session starts with mode selection before any project work. Interactive or autonomous. Writes to ~/.claude/.silver-bullet/mode. In autonomous mode pre-answer model routing worktree agent team decisions upfront using the host execution tier by default.' },
  ...
  { page:'Skills', url:'/help/reference/', anchor:'sb-skills',
    title:'Orchestration skills — silver:clarify, silver:feature, silver:bugfix, silver:ui, silver:devops, silver:research, silver:release, silver:fast',
    text:'Eight named orchestration workflows: /silver:clarify for idea-to-discussion intake, /silver:feature for feature development, /silver:bugfix for bug investigation and fixes, /silver:ui for UI/UX work, /silver:devops for infrastructure and DevOps tasks, /silver:research for research and exploration, /silver:release for release preparation, /silver:fast for quick low-overhead tasks. Invoked directly or routed automatically by /silver.' },
