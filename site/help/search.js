...
var IDX = [
  ...
  { page:'Core Concepts', url:'/help/concepts/', anchor:'enforcement',
    title:'11 enforcement layers — Stop hook, UserPromptSubmit reminder, forbidden-skill gate, Pre+PostToolUse hooks, GSD hooks, ROADMAP freshness gate, redundant instructions',
    text:'11 enforcement layers: 1 Skill tracker records invocations. 2 Stage enforcer hard stop if phase out of order. 3 Compliance status shows progress on every tool use. 4 Completion audit two-tier: commits and pushes require quality gates; PR creation deploy release require full required skill list. 5 CI gate blocks push if CI is failing. 6 Stop hook blocks task completion when required skills are missing — survives compaction. 7 UserPromptSubmit reminder re-injects missing skills and core rules before every message. 8 Forbidden skill gate blocks deprecated skills before execution. 9 GSD workflow guard detects edits outside /gsd commands. 10 ROADMAP freshness gate blocks git commit if phase SUMMARY.md staged but ROADMAP.md checkbox not ticked. 11 Redundant instructions plus anti-rationalization.' },
