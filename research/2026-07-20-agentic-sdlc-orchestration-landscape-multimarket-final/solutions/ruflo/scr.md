# Solution Capability Report: ruflo
...
- Ruflo (formerly Claude Flow, by ruvnet) packages the SPARC methodology — Specification, Pseudocode, Architecture, Refinement, Completion — as a test-driven process layer over Claude Code.
- Ruflo uses a hive-mind topology (queen agent coordinating specialised sub-agents) with 4 swarm topologies and 87 MCP tools, and was rewritten in Rust with persistent memory and MCP bridging — strong specialist_agents and cross_session_state scores.
...
- Ruflo / Claude Flow orchestrates swarms of specialist agents with cross-session state; satisfies specialist_agents + cross_session_state + plugin_skill_hook_packaging. Lifecycle coverage is broad; gate enforcement varies.
...
- Ruflo / Claude Flow orchestrates swarms of specialist agents with cross-session state; satisfies specialist_agents + cross_session_state + plugin_skill_hook_packaging. Lifecycle coverage is broad; gate enforcement varies.
- APO secondary market (SDLC plugins) is formidable substitutes territory: BMAD, GSD, Superpowers, Spec Kit, SuperClaude, Ruflo, OMP, Zuvo collectively cover plan→spec→build→test→review→ship with varying gate depth. Solution cards must include them, not demote to Adjacent.
- Ruflo / Claude Flow provides OSS workflow orchestration for Claude with multi-agent coordination and SDLC process layers — secondary-market substitute with lifecycle_span.
...
- Ruflo/Claude Flow qualifies as secondary-market SDLC plugin — workflow orchestration layer above Claude
