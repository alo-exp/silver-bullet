...
- **Subagent model (Cursor)** — All Task/subagent delegations (Multitask Mode, background workers, review ladders, site workers, explore agents, and nested workers) MUST default to **Grok 4.6 Medium** (`cursor-grok-4.6-medium`; Silver Bullet `subagent_type`: `sb-grok-4-6-medium`) unless the user explicitly requests another model for that task **or** the task requires another family (RFL GPT/Claude/OpenCode rungs, visual Gemini, etc.). Interrupt and relaunch generic mismatched workers onto Medium. Never use Fast as the default. If the host rejects `cursor-grok-4.6-medium`, use the closest available non-Fast Grok 4.6 slug; do not treat Extra High as the default. Global policy: `~/.cursor/AGENTS.md` and `~/.cursor/rules/subagent-grok-4.6-medium.mdc` (keep `no-fast-mode.mdc`).
