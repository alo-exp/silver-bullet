...
- **Subagent model (Cursor)** — When the main session uses **GPT-5.6 Sol High**, all Task/subagent delegations (Multitask Mode, background workers, review ladders, site workers, explore agents, and nested workers) MUST use **`gpt-5.6-sol-high` only**. Interrupt and relaunch mismatched workers. For other main-session models, default to `composer-2.5` unless the user explicitly requests another available model. Never use `composer-2.5-fast` or a Grok Fast variant. Global policy: `~/.cursor/AGENTS.md` and `~/.cursor/rules/subagent-composer-2.5-only.mdc`.
