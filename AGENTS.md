...
- **Subagent model (Cursor)** — All Task/subagent delegations (Multitask Mode, background workers, review ladders, site workers, explore agents, and nested workers) MUST default to **Cursor Grok 4.6 Extra High** (`cursor-grok-4.6-xhigh`) unless the user explicitly requests another model for that task. Interrupt and relaunch mismatched workers. Never use `composer-2.5`, `composer-2.5-fast`, Grok 4.5 High, or a Grok Fast variant as the default. If the host rejects `cursor-grok-4.6-xhigh`, use `cursor-grok-4.6-high` rather than falling back to 4.5. Global policy: `~/.cursor/AGENTS.md` and `~/.cursor/rules/subagent-grok-4.6-extra-high.mdc` (keep `no-grok-4.5-fast.mdc`).
