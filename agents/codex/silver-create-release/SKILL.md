...
- `git add CHANGELOG.md README.md .codex-plugin/marketplace.json` (stage release doc + marketplace updates — Step 5c)
...
After the helper runs, the version in `.codex-plugin/marketplace.json` must match `.codex-plugin/plugin.json`, and the upstream marketplace repo must have the same version committed and pushed.
