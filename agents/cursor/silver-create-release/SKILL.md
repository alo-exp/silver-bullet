...
- `git add CHANGELOG.md README.md .cursor-plugin/marketplace.json plugins/silver-bullet/.cursor-plugin/plugin.json` (stage release doc + marketplace updates — Step 5c)
...
After the wrapper runs, the version in `.cursor-plugin/marketplace.json` must match `.cursor-plugin/plugin.json`, the version in `plugins/silver-bullet/.cursor-plugin/plugin.json` must match `$VERSION`, and both upstream marketplace repos must have the same version committed and pushed.
...
```bash
git add CHANGELOG.md README.md .cursor-plugin/marketplace.json plugins/silver-bullet/.cursor-plugin/plugin.json
