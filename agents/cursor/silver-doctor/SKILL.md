...
Log friction in `$HOME/.cursor/.silver-bullet/sb-friction-log.md` when doctor surfaces hook or install issues.
...
- D12 `$HOME/.cursor/.silver-bullet` writable
- D13 no `.claude/plugins` in Cursor hooks; `agents/cursor/` in active cache
