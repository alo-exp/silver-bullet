...
settings_path = os.path.join(os.path.expanduser('~'), '.cursor', 'settings.json')
...
def sub_path(obj, install_path):
    if isinstance(obj, str):
        return obj.replace('${CURSOR_PLUGIN_ROOT}', install_path)
...
def is_stale_sb_hook(hook):
    ...
    if '${CURSOR_PLUGIN_ROOT}/hooks/' in cmd:
        return True
