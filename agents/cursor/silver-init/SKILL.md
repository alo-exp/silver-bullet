...
- **Never delete files or directories** in the project (only `$HOME/.cursor/.silver-bullet/` state files are deleted)
...
Run this phase exactly once per session. Skip if the session state file `$HOME/.cursor/.silver-bullet/session-init` already exists.

```bash
test -f $HOME/.cursor/.silver-bullet/session-init && echo "ALREADY_DONE" || echo "NEEDED"
```
...
```bash
touch $HOME/.cursor/.silver-bullet/session-init
```
...
Then use direct user interaction:
...
If the command fails (exit code non-zero), use direct user interaction:
...
|------|----------------|----------------------|
| Cursor | Add marketplace `https://github.com/alo-labs/claude-plugins`, then `/plugin install silver-bullet@alo-labs` | `bash scripts/install-claude.sh` |
...
runtime tier. Cursor stores SB state under `~/.cursor/.silver-bullet/` and
merges hooks into `~/.cursor/hooks.json`.
...
test -f "$HOME/.cursor/plugins/cache/superpowers-marketplace/superpowers/current/skills/brainstorming/SKILL.md" || \
  find "$HOME/.cursor/plugins/cache" -path '*/superpowers/*/skills/brainstorming/SKILL.md' -print -quit 2>/dev/null | grep -q .
printf '%s\n' "$?"

printf 'legacy-lifecycle-plugin: '
{ test -f "$HOME/.cursor/get-shit-done/workflows/new-project.md" || test -f "$HOME/.cursor/get-shit-done/bin/gsd-tools.cjs" || test -f "$HOME/.cursor/commands/gsd/new-project.md"; }
printf '%s\n' "$?"

printf 'legacy-knowledge-work-plugin: '
find "$HOME/.cursor/plugins/cache" \( -path '*/knowledge-work-plugins/*/engineering/skills/*' -o -path '*/knowledge-work-plugins/*/design/skills/*' -o -path '*/knowledge-work-plugins/*/product-management/skills/*' \) -print -quit 2>/dev/null | grep -q .
printf '%s\n' "$?"
```
...
- In Cursor, hooks live in `~/.cursor/hooks.json`; skills are invoked through the Cursor Skill tool or `silver-bullet invoke-skill`.
...
Use the Read tool to read `.cursor/settings.json` in the project root. If the file does not exist, skip this check.
...
> ⚠️ Incompatible v1 Silver Bullet hooks detected in `.cursor/settings.json`.
...
- Question: "Remove these incompatible v1 hook entries from .cursor/settings.json?"
- Options:
  - "A. Yes, remove them"
  - "B. No, stop init"

If user selects A, use the Edit tool to remove the offending hook entries from `.cursor/settings.json`. If user selects B, STOP.
...
```bash
cat "$HOME/.cursor/plugins/installed_plugins.json" | jq -r '.plugins["silver-bullet@silver-bullet"][0].version // "unknown"'
```
...
If installed < latest, use direct user interaction:
...
Read installed versions from `$HOME/.cursor/plugins/installed_plugins.json`. Display the installed version of each plugin found:

```bash
cat "$HOME/.cursor/plugins/installed_plugins.json" | jq -r '
  .plugins | to_entries[] |
...
Read installed version:
```bash
cat "$HOME/.cursor/plugins/installed_plugins.json" | jq -r '.plugins["multai@multai"][0].version // "unknown"'
```

Check latest:
```bash
cat "$HOME/.cursor/plugins/cache/multai/CHANGELOG.md" 2>/dev/null | grep "^## \[" | head -1
```
...
No direct user interaction needed — MultAI update is user-initiated only. Display the notice and continue.
...
If `NOT_GIT`, use direct user interaction:
...
Check if `.cursor/settings.local.json` has a `permissions.defaultMode` set:
```bash
test -f .cursor/settings.local.json && jq -r '.permissions.defaultMode // "NOT_SET"' .cursor/settings.local.json 2>/dev/null || echo "NOT_SET"
```
...
- Read `.cursor/settings.local.json` (create if absent with `{"permissions":{}}`)
...
Only use direct user interaction if the detection is genuinely ambiguous:
...
Project has: `silver-bullet.md`, `.silver-bullet.json`, `docs/workflows/*.md`, placeholder `docs/*.md`, an initial git commit, SB hooks registered in `$HOME/.cursor/settings.json`, and an activation message printed. If the project already had a project instruction file, it was updated in place; otherwise no new project instruction file was created.
...
- **3.1c Conflict resolution** (only when an existing project instruction file is present — no silent override guarantee):
  ...
  **3.1c-3 For each SB-owned section that differs**, use direct user interaction with three options:
  ...
  **3.1c-4 Apply decisions in order:**
  ...
  - Merge: display both versions in full. Ask the user via direct user interaction: "Paste or describe your merged version for the **{section-heading}** section" with options "A. Use existing (same as Keep)  B. Use template (same as Replace)  C. I'll paste the merged text below". If C is selected, read the user's next free-form message as the merged content and write it as the section body.
...
- **`scripts/merge-cursor-hooks.py`** — Idempotent hook merge script for Phase 3.7.5 (substitutes CLAUDE_PLUGIN_ROOT, deduplicates entries)
- **`scripts/merge-cursor-hooks.py`** — Cursor hook merge for `~/.cursor/hooks.json` (substitutes CURSOR_PLUGIN_ROOT)
