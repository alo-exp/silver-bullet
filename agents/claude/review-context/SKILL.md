---
name: review-context
description: This skill should be used for CONTEXT.md artifact reviewer — validates that all discussion points are resolved (locked decision or Claude Code's Discretion), decisions are specific (not vague), and no contradictions exist between decisions
...
**If missing or empty:** Emit ISSUE finding `CTX-F01` with suggestion = "Add a `## Decisions` section listing at least one resolved decision or marking items as Claude Code's Discretion."
...
- **Claude Code's Discretion** — explicitly marked as delegated to Claude Code

A gray area is unresolved if it is listed without any resolution or still phrased as a question with no answer below it.

**If any unresolved gray area found:** Emit ISSUE finding `CTX-F02` (increment suffix for each instance) with location = the unresolved item, suggestion = "Resolve this point — either record the concrete decision made or mark it as `Claude Code's Discretion: [context for Claude Code to decide]`."
...
### QC-6: Claude Code's Discretion Items Have Sufficient Context

Any item marked as "Claude Code's Discretion" MUST include enough context for Claude Code to make a reasonable, informed choice. A bare "Claude Code's Discretion" label without description or constraints fails this check.

**If any Claude Code's Discretion item lacks context:** Emit ISSUE finding `CTX-F40` with location = the item, suggestion = "Add context to guide Claude Code's choice, e.g., constraints, preferences, or the options being considered."
