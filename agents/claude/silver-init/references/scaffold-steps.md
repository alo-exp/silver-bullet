...
Merges the SB hook entries into the user's global host settings file so hooks remain active in projects installed without the marketplace (e.g. manual installs, workspace clones). Target the active host's settings path (`~/.claude/settings.json` for Claude, `$HOME/.claude/hooks.json` for Cursor via `merge-cursor-hooks.py`, Codex native plugin hooks when applicable).
...
import json, os, sys
reg = os.path.expanduser('$HOME/.claude/plugins/installed_plugins.json')
with open(reg) as f:
    data = json.load(f)
