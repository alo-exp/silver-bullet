...
if [[ -n "${SILVER_BULLET_RUNTIME:-}" ]]; then
  ...
  SB_HOST=codex
elif [[ -n "${CLAUDE_PLUGIN_ROOT:-}" && "$CLAUDE_PLUGIN_ROOT" == *"/.claude/"* ]]; then
  SB_HOST=codex
elif [[ -n "${CLAUDE_PLUGIN_ROOT:-}" && "$CLAUDE_PLUGIN_ROOT" == *"/.claude/"* ]]; then
  SB_HOST=cursor
else
...
|------------------|----------------------|----------|
| `claude` | `graphify claude install --project` | `.claude/settings.json` hooks |
| `codex` | `graphify codex install --project` | `.claude/hooks.json` |
...
- **3.2 Create dirs**: `mkdir -p docs/specs docs/workflows .silver-bullet/orchestrator-workers .claude/rules`.
