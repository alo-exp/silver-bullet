...
runtime tier. Cursor stores SB state under `$HOME/.claude/.silver-bullet/` and
merges hooks into `$HOME/.claude/hooks.json`.
...
- In Cursor, hooks live in `$HOME/.claude/hooks.json`; skills are invoked through the Cursor Skill tool or `silver-bullet invoke-skill`.
...
- **`scripts/merge-cursor-hooks.py`** — Cursor hook merge for `$HOME/.claude/hooks.json` (substitutes CURSOR_PLUGIN_ROOT)
