...
```bash
export SB_RESEARCH_OUT_DIR="research/$(date -u +%Y-%m-%d)-<slug>"
...
4. Write `run_manifest.json` with:
   ...
   - `question`
   - `mode`
   - `research_type` (`default` | `solution-landscape` | `solution-compare`)
   - `started_at`
   - `search_cli.status`
...
`interview_complete: true` and `license_preference` set.
...
- `reference/need-profile-interview.md` for solution decision interviews
- `reference/landscape/` for market landscape prompts
- `reference/solution-scr/scr-guide.md` for Solution Capability Reports (SCR)
- `reference/comparator/` for weighted matrix methodology
...
Legacy `.planning/research/` paths are archived; new runs use `research/` at repo root.

Required (mode-dependent; see `phases.yaml` + `research_types`):
...
Solution-landscape / solution-compare also require:
...
Landscape adds `landscape/landscape-report.md`, `shortlist/shortlist.json` (exactly 5).
Compare adds `solutions_requested.json` (N ≥ 2); no shortlist.
...
python3 skills/silver-deep-research/scripts/phase_gate.py --dir "$SB_RESEARCH_OUT_DIR"
```

For solution types, also run:
