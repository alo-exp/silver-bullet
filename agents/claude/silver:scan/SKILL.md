...
- scans `docs/sessions/*.md` plus any relevant Codex, Claude Code, or SB-maintained session/handoff logs it can safely identify
- supports `--agent-env auto|Codex|Claude Code|kay` to select the runtime session store family explicitly
...
1. Run `scripts/silver-scan.sh scan --mode <interactive|autonomous> --agent-env <auto|Codex|Claude Code|kay> --format json`.
