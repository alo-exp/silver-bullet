...
- Claude Code-style slash/skill aliases may expose `silver:feature`.
...
(persisted in `$HOME/.claude/.silver-bullet/orchestrator.json`). Store the user prompt in
`$HOME/.claude/.silver-bullet/orchestrator-intent.txt` for hook consumption.
