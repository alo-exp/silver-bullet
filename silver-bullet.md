...
1. **Switch to the host execution model** if not already selected. Claude uses `claude-sonnet-4-6`; Codex uses `gpt-5.3-medium`.
...
**Host-aware model ladder:** See [`docs/RUNTIME-COMPATIBILITY.md`](docs/RUNTIME-COMPATIBILITY.md) for the exact per-host map. The session model is the host execution tier by default.

| Category | Claude host | Codex host | Examples |
|----------|-------------|------------|---------|
| Structured output | Haiku | `GPT-5.2-low` | codebase-mapper, intel-updater, user-profiler, assumptions-analyzer |
| Execution, Research | `claude-sonnet-4-6` | `GPT-5.3-medium` | executor, phase-researcher, doc-writer, code-fixer |
| Design, Review, Verification | `claude-opus-4-6` | `GPT-5.4-high` | planner, roadmapper, plan-checker, code-reviewer, verifier |
| Deep reasoning / hardest architecture / adversarial security | `claude-opus-4-6` | `GPT-5.5-xhigh` | security-auditor, debugger, especially thorny planning calls |

**Session model:** Use the host execution tier for all inline work, interactive skills, and user-facing conversation.
...
**For inline skills** (design, product-management, engineering, MultAI, etc.) that run in the current session: keep using the host session directly. Silver Bullet composes the workflow; it does not select subagent models.
