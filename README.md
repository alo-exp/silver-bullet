...
**Current version: v0.19.1** — Hooks now exit silently in non-Silver-Bullet project directories (fixes noisy hook errors in every non-GSD session). Also cleans up hook internals: removes the redundant `quality-gate-stage-N` enforcement mechanism and the unused `is_release` variable. Also v0.19.0: `silver:brainstorm-idea` orchestrator for idea-stage projects (SPEC.md-absent routing). Previous: v0.18.4 removes redundant `hooks` field from `plugin.json`; v0.18.3 fixes F-07 hook false-positive for interpreter binaries; v0.18.1 adds configurable review depth, review analytics, and cross-artifact consistency validation.
