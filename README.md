...
**Current version: v0.13.0** — SENTINEL v2.3 security audit applied: UNTRUSTED DATA boundaries added to §0 doc-read and `silver:init`, `mode` file added to state tamper prevention, `silver:update` now requires commit SHA confirmation before registry write, §10 preference writes require diff display + explicit confirmation. Plus 7 new orchestration skills (`silver:feature`, `silver:bugfix`, `silver:ui`, `silver:devops`, `silver:research`, `silver:release`, `silver:fast`), SB Orchestrated Workflows (§2h), expanded `/silver` router, `silver:init` plugin detection, §10 User Workflow Preferences.
